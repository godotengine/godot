...
def make_donors_header(target, source, env):
    ...
    sections_id = ["DONORS_SPONSOR_PLAT", "DONORS_SPONSOR_GOLD", "DONORS_SPONSOR_MINI", "DONORS_GOLD", "DONORS_SILVER", "DONORS_BRONZE"]
...
def make_license_header(target, source, env):
    ...
    with open_utf8(dst, "w") as f:
        ...
        for project_name, project in iteritems(projects):
            ...
            for part in project:
                f.write('\t{ "' + escape_string(part["License"][0]) + '", ' + "&COPYRIGHT_INFO_DATA[" + str(part["file_index"]) + "], " + "&COPYRIGHT_INFO_DATA[" + str(part["copyright_index"]) + "], " + str(len(part["Files"])) + ", " + str(len(part["Copyright"])) + " },\n")
                part_index += 1
        f.write("};\n\n")
        ...
        for project_name, project in iteritems(projects):
            f.write('\t{ "' + escape_string(project_name) + '", ' + "&COPYRIGHT_PROJECT_PARTS[" + str(part_indexes[project_name]) + "], " + str(len(project)) + " },\n")
        f.write("};\n\n")
