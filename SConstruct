...
opts.Add(
    BoolVariable(
        "disable_advanced_gui", "Disable advanced 3D GUI nodes and behaviors", False
    )
)
opts.Add(
    "extra_suffix",
...
opts.Add(
    EnumVariable(
        "warnings",
        "Set the level of warnings emitted during compilation",
        "no",
...
opts.Add(
    BoolVariable(
        "no_editor_splash", "Don't use the custom splash screen for the editor", False
    )
)
opts.Add(
    "system_certs_path",
...
opts.Add(
    BoolVariable(
        "builtin_libwebsockets", "Use the built-in libwebsockets library", True
    )
)
...
opts.Add(
    BoolVariable("builtin_thekla_atlas", "Use the built-in thekla_altas library", True)
)
...
if selected_platform in platform_list:
    ...
    if env.msvc:
        disable_nonessential_warnings = [
            ...
            "/wd4305",
            "/wd4800",
        ]  # Truncations, narrowing conversions...
