...
template_only_msg = "Build option `{}=yes` cannot be used for editor builds, only for export template builds."
if env["disable_3d"]:
    if env.editor_build:
        print_error(template_only_msg.format("disable_3d"))
        Exit(255)
    else:
...
if env["disable_advanced_gui"]:
    if env.editor_build:
        print_error(template_only_msg.format("disable_advanced_gui"))
        Exit(255)
    else:
        env.Append(CPPDEFINES=["ADVANCED_GUI_DISABLED"])
if env["disable_physics_2d"]:
    if env.editor_build:
        print_error(template_only_msg.format("disable_physics_2d"))
        Exit(255)
    else:
        env.Append(CPPDEFINES=["PHYSICS_2D_DISABLED"])
if env["disable_physics_3d"]:
    if env.editor_build:
        print_error(template_only_msg.format("disable_physics_3d"))
        Exit(255)
    else:
