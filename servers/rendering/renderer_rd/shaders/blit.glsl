...
void main() {
	...
	if (data.target_color_space == COLOR_SPACE_REC709_NONLINEAR_SRGB) {
		...
		// Adjust brightness to the destination buffer's reference luminance
		color.rgb *= data.reference_multiplier;

		if (data.target_color_space == COLOR_SPACE_REC2020_NONLINEAR_ST2084) {
			// Convert to Rec. 2020 primaries
			color.rgb = rec709_to_rec2020(color.rgb);
			// Apply the ST2084 (PQ) curve
