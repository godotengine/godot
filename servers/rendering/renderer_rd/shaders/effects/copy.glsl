...
void main() {
	...
	vec4 color = textureLod(source_color, vec3_to_oct_with_border(normal, params.octmap_pixel_size), params.camera_z_far); //the biggest the lod the least the acne
#else
	vec4 color = textureLod(source_color, vec3(vec3_to_oct_with_border(normal, params.octmap_pixel_size), params.camera_z_far), 0.0); //the biggest the lod the least the acne
#endif
	imageStore(dest_buffer, pos + params.target, color);
