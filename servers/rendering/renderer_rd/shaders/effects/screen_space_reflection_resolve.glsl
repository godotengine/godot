...
#ifdef MODE_ROUGH
layout(set = 1, binding = 0) uniform sampler2D source_mip_level;
#endif
...
void get_sample(float depth, vec3 normal, float roughness, ivec2 pixel_pos, out vec4 color, out float weight) {
	float sample_depth = texelFetch(source_depth, pixel_pos * 2, 0).x;
	vec4 sample_normal_roughness = texelFetch(source_normal_roughness, pixel_pos * 2, 0);
...
void main() {
	...
	vec2 half_tex_coord = (pixel_pos + 0.5) * 0.5 - 0.5;
	vec2 bilinear_weights = fract(half_tex_coord);

	vec4 color0, color1, color2, color3;
	float weight0, weight1, weight2, weight3;

	get_sample(depth, normal, roughness, (pixel_pos / 2) + ivec2(0, 0), color0, weight0);
	get_sample(depth, normal, roughness, (pixel_pos / 2) + ivec2(1, 0), color1, weight1);
	get_sample(depth, normal, roughness, (pixel_pos / 2) + ivec2(0, 1), color2, weight2);
	get_sample(depth, normal, roughness, (pixel_pos / 2) + ivec2(1, 1), color3, weight3);
	...
	result_color /= max(0.00001, weight0 + weight1 + weight2 + weight3);

#ifdef MODE_ROUGH
	// Invert the tone mapping we applied in the main trace pass.
	result_color.rgb /= 1.0 - dot(result_color.rgb, vec3(0.299, 0.587, 0.114));
#endif
