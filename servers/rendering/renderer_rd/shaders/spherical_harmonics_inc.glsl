...
hvec3 evaluate_sh_l1(hvec3 dir, vec4 sh_coeffs[3]) {
...
mediump half shEvaluateDiffuseL1Geomerics(hvec3 n, vec4 sh_coeffs[3], int channel) {
...
hvec3 evaluate_sh_l1_geomerics(hvec3 n, vec4 sh_coeffs[3]) {
...
hvec3 evaluate_sh_l1_geomerics_inlined(hvec3 dir, vec4 sh_coeffs[3]) {
...
hvec3 evaluate_sh_l1_geomerics(hvec3 dir, vec4 sh_coeffs[5]) {
	// Load from uniforms.
	...
	// Actual math.
	const hvec3 q = half(0.5) * (half(1.0) + hvec3(dot(R1_div_lenR1[0], dir), // r
