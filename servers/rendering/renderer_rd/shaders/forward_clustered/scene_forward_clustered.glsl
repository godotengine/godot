...
vec4 fog_process(vec3 vertex) {
	...
	if (scene_data_block.data.fog_aerial_perspective > 0.0) {
		...
		vec2 cube_uv = vec3_to_oct_with_border(cube_view, scene_data_block.data.radiance_pixel_size);
		vec2 cube_grad = vec3_to_oct_with_border_and_gradient(cube_view, scene_data_block.data.radiance_pixel_size, true);
		vec3 sky_sample_a = textureGrad(sampler2DArray(radiance_octmap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3(cube_uv, lod), dFdx(cube_grad), dFdy(cube_grad)).rgb;
		vec3 sky_sample_b = textureGrad(sampler2DArray(radiance_octmap, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3(cube_uv, lod + 1), dFdx(cube_grad), dFdy(cube_grad)).rgb;
