...
vec4 fog_process(vec3 vertex) {
	...
	if (scene_data_block.data.fog_aerial_perspective > 0.0) {
		...
		vec2 cube_uv = vec3_to_oct_with_border(cube_view, scene_data_block.data.radiance_pixel_size * pow(2.0, cube_lod));
		...
		vec2 cube_uv = vec3_to_oct_with_border(cube_view, scene_data_block.data.radiance_pixel_size * pow(2.0, roughness_lod));
...
// Use bent normal for indirect lighting where possible
	...
	if (bool(scene_data.flags & SCENE_DATA_FLAGS_USE_REFLECTION_CUBEMAP)) {
		...
		vec2 ref_uv = vec3_to_oct_with_border(ref_vec, scene_data_block.data.radiance_pixel_size * pow(2.0, ref_lod));
		...
		vec2 ref_uv = vec3_to_oct_with_border(ref_vec, scene_data_block.data.radiance_pixel_size * pow(2.0, roughness_lod));
	...
	if (bool(scene_data.flags & SCENE_DATA_FLAGS_USE_AMBIENT_LIGHT)) {
		...
		if (bool(scene_data.flags & SCENE_DATA_FLAGS_USE_AMBIENT_CUBEMAP)) {
			...
			vec2 ambient_uv = vec3_to_oct_with_border(ambient_dir, scene_data_block.data.radiance_pixel_size * pow(2.0, ambient_lod));
			...
			vec2 ambient_uv = vec3_to_oct_with_border(ambient_dir, scene_data_block.data.radiance_pixel_size * pow(2.0, roughness_lod));
