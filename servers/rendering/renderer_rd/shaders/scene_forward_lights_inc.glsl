...
void reflection_process(uint ref_index, vec3 vertex, hvec3 ref_vec, hvec3 normal, half roughness, hvec3 ambient_light, hvec3 specular_light, inout hvec4 ambient_accum, inout hvec4 reflection_accum) {
	...
	if (reflections.data[ref_index].intensity > 0.0 && reflection_accum.a < half(1.0)) { // compute reflection
		...
		vec2 reflection_uv = vec3_to_oct_with_border(local_ref_vec, border_size * pow(2.0f, roughness_lod));
		reflection.rgb = hvec3(textureLod(sampler2DArray(reflection_atlas, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3(reflection_uv, reflections.data[ref_index].index), roughness_lod).rgb) * sc_luminance_multiplier();
	...
	switch (reflections.data[ref_index].ambient_mode) {
		...
		case REFLECTION_AMBIENT_ENVIRONMENT: {
			...
			vec2 ambient_uv = vec3_to_oct_with_border(local_amb_vec, border_size * pow(2.0f, roughness_lod));
