...
void reflection_process(uint ref_index, vec3 vertex, hvec3 ref_vec, hvec3 normal, half roughness, hvec3 ambient_light, hvec3 specular_light, inout hvec4 ambient_accum, inout hvec4 reflection_accum) {
	...
	vec2 border_size = scene_data_block.data.reflection_atlas_pixel_size;
	if (reflections.data[ref_index].intensity > 0.0 && reflection_accum.a < half(1.0)) { // compute reflection
		...
		reflection.rgb = hvec3(textureLod(sampler2DArray(reflection_atlas, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3(vec3_to_oct_with_border(local_ref_vec, border_size), reflections.data[ref_index].index), sqrt(roughness) * MAX_ROUGHNESS_LOD).rgb) * sc_luminance_multiplier();
	...
	switch (reflections.data[ref_index].ambient_mode) {
		...
		case REFLECTION_AMBIENT_ENVIRONMENT: {
			...
			ambient_out.rgb = hvec3(textureLod(sampler2DArray(reflection_atlas, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3(vec3_to_oct_with_border(local_amb_vec, border_size * pow(2.0f, MAX_ROUGHNESS_LOD)), reflections.data[ref_index].index), MAX_ROUGHNESS_LOD).rgb);
