...
#endif
#ifdef LIGHT_SHEEN_USED
		hvec3 s_ref_vec, half s_roughness, inout hvec4 s_reflection_accum,
#endif
...
#ifdef LIGHT_SHEEN_USED
	if (reflections.data[ref_index].intensity > 0.0 && s_reflection_accum.a < half(1.0)) { // compute sheen reflection
		...
		hvec4 s_reflection;
		half s_reflection_blend = max(half(0.0), blend - s_reflection_accum.a);
		...
		s_reflection.rgb = hvec3(textureLod(sampler2DArray(reflection_atlas, DEFAULT_SAMPLER_LINEAR_WITH_MIPMAPS_CLAMP), vec3(s_reflection_uv, reflections.data[ref_index].index), s_roughness_lod).rgb) * REFLECTION_MULTIPLIER;
		s_reflection.rgb *= half(reflections.data[ref_index].exposure_normalization);
		s_reflection.a = s_reflection_blend;

		s_reflection.rgb *= half(reflections.data[ref_index].intensity);
		s_reflection.rgb *= s_reflection.a;
