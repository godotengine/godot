...
float sample_directional_pcf_shadow(texture2D shadow, vec2 shadow_pixel_size, vec4 coord, float jitter_fraction) {
	vec2 pos = coord.xy;
	float depth = coord.z;
	...
	mat2 disk_rotation;
	{
		float r = (quick_hash(gl_FragCoord.xy) + jitter_fraction) * 2.0 * M_PI;
		float sr = sin(r);
		float cr = cos(r);
...
float sample_pcf_shadow(texture2D shadow, vec2 shadow_pixel_size, vec3 coord, float jitter_fraction) {
	vec2 pos = coord.xy;
	float depth = coord.z;
...
float sample_omni_pcf_shadow(texture2D shadow, float blur_scale, vec2 coord, vec4 uv_rect, vec2 flip_offset, float depth, float jitter_fraction) {
	//if only one sample is taken, take it from the center
	if (sc_soft_shadow_samples == 0) {
...
float sample_directional_soft_shadow(texture2D shadow, vec3 pssm_coord, vec2 tex_scale, float jitter_fraction) {
	//find blocker
...
float light_process_omni_shadow(uint idx, vec3 vertex, vec3 normal, float jitter_fraction) {
#ifndef SHADOWS_DISABLED
	if (omni_lights.data[idx].shadow_opacity > 0.001) {
		...
		if (sc_use_light_soft_shadows && omni_lights.data[idx].soft_shadow_size > 0.0) {
			...
			depth = 1.0 - depth;
			shadow = mix(1.0, sample_omni_pcf_shadow(shadow_atlas, omni_lights.data[idx].soft_shadow_scale / shadow_sample.z, pos, uv_rect, flip_offset, depth, jitter_fraction), omni_lights.data[idx].shadow_opacity);
		}
...
float light_process_spot_shadow(uint idx, vec3 vertex, vec3 normal, float jitter_fraction) {
#ifndef SHADOWS_DISABLED
	if (spot_lights.data[idx].shadow_opacity > 0.001) {
		...
		if (sc_use_light_soft_shadows && spot_lights.data[idx].soft_shadow_size > 0.0) {
			...
			shadow = mix(1.0, sample_pcf_shadow(shadow_atlas, spot_lights.data[idx].soft_shadow_scale * scene_data_block.data.shadow_atlas_pixel_size, shadow_uv, jitter_fraction), spot_lights.data[idx].shadow_opacity);
		}
