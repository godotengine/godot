...
struct InstanceData {
	...
	vec4 compressed_aabb_position_pad; // 16 - 112 // Only .xyz is used. .w is padding.
	vec4 compressed_aabb_size_pad; // 16 - 128 // Only .xyz is used. .w is padding.
	vec4 uv_scale; // 16 - 144

	uvec2 reflection_probes; // 08 - 152
	uvec2 omni_lights; // 08 - 160
	uvec2 spot_lights; // 08 - 168
	uvec2 area_lights; // 08 - 176

	uvec2 decals; // 08 - 184
	uvec2 padding; // 08 - 192
};
