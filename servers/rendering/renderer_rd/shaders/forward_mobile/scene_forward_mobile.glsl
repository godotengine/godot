...
void calculate_view_params(in vec3 position, out vec3 view, out vec3 eye_offset) {
#ifdef USE_MULTIVIEW
...
void main() {
	...
	calculate_view_params(surface_pos, view, eye_offset);
...
// If alpha scissor is used, we must further the edge threshold, otherwise we won't get any edge feather
	...
	if (!sc_disable_reflection_probes) { //Reflection probes
		...
		for (uint i = 0; i < 8; i++) {
			...
			reflection_process(reflection_index, surface_pos, ref_vec, bent_normal, roughness, ambient_light, specular_light, ambient_accum, reflection_accum);
		}
...
#define BIAS_FUNC(m_var, m_idx)                                                                                                                                       \
	...
	vec3 normal_bias = normalize(surface_normal) * (1.0 - max(0.0, dot(light_dir, -normalize(surface_normal)))) * directional_lights.data[i].shadow_normal_bias[m_idx]; \
