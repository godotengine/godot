...
#define LIGHTPROBE_OCT_SIZE HDDAGI_OCT_SIZE
...
void sdfvoxel_gi_process(int cascade, vec3 cascade_pos, vec3 cam_pos, vec3 cam_normal, vec3 cam_specular_normal, float roughness, out vec3 diffuse_light, out vec3 specular_light) {
	...
	for (int i = 0; i < 8; i++) {
		...
		vec2 tex_uv = (base_tex_uv + light_uv) * light_probe_tex_to_uv;
		...
		tex_uv = (base_tex_uv + light_uv_spec) * light_probe_tex_to_uv;
		vec3 probe_ref_light = roughness < 0.99 ? texture(sampler2DArray(hddagi_lightprobe_specular, SAMPLER_LINEAR_CLAMP), vec3(tex_uv, float(cascade))).rgb : vec3(0.0);
		vec3 probe_ref_full_light = roughness > ROUGHNESS_TO_REFLECTION_TRESHOOLD ? texture(sampler2DArray(hddagi_lightprobe_diffuse, SAMPLER_LINEAR_CLAMP), vec3(tex_uv, float(cascade))).rgb : vec3(0.0);
...
void hddagi_process(vec3 vertex, vec3 normal, vec3 reflection, float roughness, out vec4 ambient_light, out vec4 reflection_light) {
	...
	if (cascade < HDDAGI_MAX_CASCADES) {
		...
		ambient_light.rgb = diffuse * hddagi.energy;
		reflection_light.rgb = specular * hddagi.energy;
	} else {
