...
layout(set = 0, binding = 4) uniform sampler linear_sampler_with_mipmaps;

layout(set = 0, binding = 5, std430) restrict readonly buffer DispatchData {
	uint x;
	uint y;
	uint z;
	uint total_count;
...
layout(set = 0, binding = 6, std430) restrict buffer ProcessVoxels {
	ProcessVoxel data[];
}
process_voxels;
...
void main() {
	...
	for (uint i = 0; i < params.light_count; i++) {
		...
		*/
		...
		if (!hit) {
			light_accum += albedo * lights.data[i].color.rgb * texture_color * lights.data[i].energy * attenuation;
		}
	}
