...
#if defined(SCROLL_HALF)
layout(local_size_x = 56, local_size_y = 1, local_size_z = 1) in;
#elif defined(SCROLL_QUARTER)
...
shared vec2 neighbours[OCCLUSION_OCT_SIZE * OCCLUSION_OCT_SIZE];
...
void main() {
	...
	if (params.scroll.y != 0) {
		...
		limit = params.grid_size.y;
	} else if (params.scroll.z != 0) {
		pos.xz = pos.zx;
		scroll = params.scroll.z;
		limit = params.grid_size.z;
	} else {
		scroll = params.scroll.x;
		limit = params.grid_size.x;
	}

	if (index >= (limit - abs(scroll))) {
		return; // out of range
	}

	pos.y += params.grid_size.y * params.cascade;
	...
	if (solid_count == 0) {
		...
		imageStore(dst_sdf,global_pos + ivec3(0,params.grid_size.y * params.cascade,0) ,vec4(encode_distance(100.0)));
		return;
	}
	...
	imageStore(dst_sdf,global_pos + ivec3(0,params.grid_size.y * params.cascade,0) ,vec4(encode_distance(d)));
	...
	if (hit) {
		...
		const vec3 aniso_dir[6] = vec3[](
			...
			vec3(0, 0, -1),
			vec3(0, 0, 1));
		bool hit_front_face =false;
		for(int i=0;i<6;i++) {
			if ( bool((1<<i) & hit_normals) && dot(aniso_dir[i],ray_dir) < 0.0 ) {
				hit_front_face = true;
			}
		}
		...
		if (!hit_front_face) {
			...
			d = max(0.0, d- 1.0 / max(abs_ray_dir.x, max(abs_ray_dir.y, abs_ray_dir.z)) * bias );
		...
		uint ray_test = 0;
		float m = abs_ray_dir.x;
		ray_test = ray_dir.x < 0 ? 1 : 0;
		if (abs_ray_dir.y < m) {
			m = abs_ray_dir.y;
			ray_test = ray_dir.y < 0 ? 3 : 2;
		}
		if (abs_ray_dir.z < m) {
			m = abs_ray_dir.z;
			ray_test = ray_dir.z < 0 ? 5 : 4;
		}

		if (!bool(hit_normals & ray_params.ray_biastest)) {
			// Hit a bafkace, go back one cell as penalty
			d = max(0.0,d - 1.0);
		...
		vec3 min_bounds = vec3(-4.0);
		vec3 max_bounds = vec3(PROBE_CELLS * 4.0 + 4.0);
	...
	ivec3 tex_uv = ivec3(probe_tex_pos * params.occ_oct_size + ivec2(gl_LocalInvocationID).xy, params.cascade);
	...
	ivec3 src_tex_uv = ivec3(probe_tex_pos * params.occ_oct_size , params.cascade);
	ivec3 dst_tex_uv = ivec3(probe_tex_pos * (params.occ_oct_size+2) + ivec2(1), params.cascade);

	float o = imageLoad(src_occlusion,src_tex_uv + ivec3(local_pos,0)).r * OCC8_DISTANCE_MAX;

	neighbours[linear_pos] = vec2(o,o*o);
	...
	for(uint i=0;i<neighbour_max_weights;i++) {
		...
		accum += neighbours[index] * weight;
	}

	vec2 occlusion = accum / OCC16_DISTANCE_MAX;
