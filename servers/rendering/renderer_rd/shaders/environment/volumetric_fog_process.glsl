...
layout(set = 1, binding = 0, std140) uniform HDDAGI {
	...
	SDFVoxelGICascadeData cascades[HDDAGI_MAX_CASCADES];
}
hddagi;

layout(set = 1, binding = 1) uniform texture2DArray hddagi_ambient_texture;

layout(set = 1, binding = 2) uniform texture3D hddagi_occlusion_texture;
...
void main() {
	...
	if (total_density > 0.00005) {
...

		{
			...
			for (uint i = 0; i < hddagi.max_cascades; i++) {
				vec3 cascade_pos = (world_pos - hddagi.cascades[i].position) * hddagi.cascades[i].to_probe;

				if (any(lessThan(cascade_pos, vec3(0.0))) || any(greaterThanEqual(cascade_pos, hddagi.cascade_probe_size))) {
				...
				tex_pos.x += probe_base_pos.z * hddagi.probe_axis_size;
				...
				for (uint j = 0; j < 8; j++) {
					...
					if (hddagi.use_occlusion) {
						ivec3 occ_indexv = abs((hddagi.cascades[i].region_world_offset + probe_posi) & ivec3(1, 1, 1)) * ivec3(1, 2, 4);
						vec4 occ_mask = mix(vec4(0.0), vec4(1.0), equal(ivec4(occ_indexv.x | occ_indexv.y), ivec4(0, 1, 2, 3)));

						vec3 occ_pos = clamp(cascade_pos, probe_pos - hddagi.occlusion_clamp, probe_pos + hddagi.occlusion_clamp) * hddagi.probe_to_uvw;
						occ_pos.z += float(i);
						if (occ_indexv.z != 0) { //z bit is on, means index is >=4, so make it switch to the other half of textures
							occ_pos.x += 1.0;
						}

						occ_pos *= hddagi.occlusion_renormalize;
						float occlusion = dot(textureLod(sampler3D(hddagi_occlusion_texture, linear_sampler), occ_pos, 0.0), occ_mask);
					...
					ivec3 uvw = tex_pos;
					uvw.xy += offset.xy;
					uvw.x += offset.z * hddagi.probe_axis_size;

					vec3 ambient = texelFetch(sampler2DArray(hddagi_ambient_texture, linear_sampler), uvw, 0).rgb;

					ambient_accum.rgb += ambient * weight * hddagi.cascades[i].exposure_normalization;
