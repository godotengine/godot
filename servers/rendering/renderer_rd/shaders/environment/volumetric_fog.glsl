...
void main() {
	...
	if (params.shape == 0) {
		...
		float inv_height = 1.0 / max(0.001, params.size.y * 2.0);
		float radius = 1.0 / max(0.001, (min(params.size.x, params.size.z)));
		...
		sdf = max(dot(c, vec2(q, local_pos.y - params.size.y / 2.0)), -params.size.y / 2.0 - local_pos.y);
	} else if (params.shape == 2) {
		// Cylinder
		// https://iquilezles.org/www/articles/distfunctions/distfunctions.htm
		vec2 d = abs(vec2(length(local_pos.xz), local_pos.y)) - vec2(min(params.size.x, params.size.z), params.size.y) / 2.0;
