...
char *(*accesskit_macos_adapter_debug_dylibloader_wrapper_libaccesskit)(const struct accesskit_macos_adapter *);
...
void (*accesskit_macos_add_focus_forwarder_to_window_class_with_length_dylibloader_wrapper_libaccesskit)(const char *, size_t);
...
// accesskit_macos_adapter_debug
  *(void **) (&accesskit_macos_adapter_debug_dylibloader_wrapper_libaccesskit) = dlsym(handle, "accesskit_macos_adapter_debug");
  if (verbose) {
    error = dlerror();
    if (error != NULL) {
      fprintf(stderr, "%s\n", error);
