...
void main() {
	...
	if (gl_VertexID % 3 == 0) {
		...
		color.xy = godot_unpackHalf2x16(read_draw_data_color_a_rg);
		color.zw = godot_unpackHalf2x16(read_draw_data_color_a_ba);
	} else if (gl_VertexID % 3 == 1) {
		vertex = read_draw_data_point_b;
		uv = read_draw_data_uv_b;
		color.xy = godot_unpackHalf2x16(read_draw_data_color_b_rg);
		color.zw = godot_unpackHalf2x16(read_draw_data_color_b_ba);
	} else {
		vertex = read_draw_data_point_c;
		uv = read_draw_data_uv_c;
		color.xy = godot_unpackHalf2x16(read_draw_data_color_c_rg);
		color.zw = godot_unpackHalf2x16(read_draw_data_color_c_ba);
	}
	...
	if (bool(read_draw_data_flags & FLAGS_INSTANCING_HAS_COLORS)) {
		vec4 instance_color;
		instance_color.xy = godot_unpackHalf2x16(uint(instance_color_custom_data.x));
		instance_color.zw = godot_unpackHalf2x16(uint(instance_color_custom_data.y));
