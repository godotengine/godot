...
#if !defined(USE_PRIMITIVE) && defined(HB_SLUG_ENABLED)
	if ((read_draw_data_flags & INSTANCE_FLAGS_SPECIAL_MODE_MASK) == INSTANCE_FLAGS_USE_SLUG || (read_draw_data_flags & INSTANCE_FLAGS_SPECIAL_MODE_MASK) == INSTANCE_FLAGS_USE_SLUG_COLOR) {
		float scale = read_draw_data_ninepatch_margins.x;
		vec4 jac = vec4(scale, 0.0, 0.0, -scale);
		vec2 viewport = vec2(1.0 / screen_pixel_size.x, 1.0 / screen_pixel_size.y);
		vec2 vertex_norm_arr[4] = vec2[](vec2(-1.0, 1.0), vec2(-1.0, -1.0), vec2(1.0, -1.0), vec2(1.0, 1.0));
		vec2 vertex_norm = vertex_norm_arr[gl_VertexID % 6];
		uv.y = -uv.y;
...
uniform highp isampler2D slug_texture; //texunit:-8
...
void main() {
...
#ifndef USE_PRIMITIVE
	if ((read_draw_data_flags & INSTANCE_FLAGS_SPECIAL_MODE_MASK) == INSTANCE_FLAGS_USE_LCD) {
...
#ifdef MSDF_ENABLED
	} else if ((read_draw_data_flags & INSTANCE_FLAGS_SPECIAL_MODE_MASK) == INSTANCE_FLAGS_USE_MSDF) {
	...
	} else if ((read_draw_data_flags & INSTANCE_FLAGS_SPECIAL_MODE_MASK) == INSTANCE_FLAGS_USE_SLUG) {
	...
	} else if ((read_draw_data_flags & INSTANCE_FLAGS_SPECIAL_MODE_MASK) == INSTANCE_FLAGS_USE_SLUG_COLOR) {
		float coverage = 0.0;
