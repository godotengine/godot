...
void vertex_shader(vec4 vertex_angle_attrib_input,
	...
	vec3 eye_offset;
	if (use_prev_multiview_data) {
		projection_matrix = multiview_data_block.prev_data.projection_matrix_view[ViewIndex];
		inv_projection_matrix = multiview_data_block.prev_data.inv_projection_matrix_view[ViewIndex];
		eye_offset = multiview_data_block.prev_data.eye_offset[ViewIndex].xyz;
	} else {
		projection_matrix = multiview_data_block.data.projection_matrix_view[ViewIndex];
		inv_projection_matrix = multiview_data_block.data.inv_projection_matrix_view[ViewIndex];
		eye_offset = multiview_data_block.data.eye_offset[ViewIndex].xyz;
	}
#else
...
#else
layout(location = 0) out vec4 motion_vector;
#endif // !RENDER_MOTION_VECTORS
...
#ifndef RENDER_MOTION_VECTORS
	// #if defined(RENDER_MOTION_VECTORS)
	// 	// Not a real output in the motion vector pass.
	// 	vec4 frag_color;
	// #endif
...
// PSSM4 shadows
	...
	motion_vector = vec4(ndc - prev_ndc, 0.0);
#endif // RENDER_MOTION_VECTORS
}
