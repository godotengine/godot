...
def save_active_platforms(apnames, ap):
    for x in ap:
        ...
        for name in svg_names:
            write_active_platform_svg(x[9:], name, x + "/" + name + ".svg", x + "/" + name + "_svg.gen.h")


def write_active_platform_svg(platform_name, svg_type_name, source_svg_filepath, output_generated_header_filepath):
    svgf = open(source_svg_filepath, "rb")
    b = svgf.read(1)
    svg_str = " /* AUTOGENERATED FILE, DO NOT EDIT */ \n"
    svg_str += " static const char *_" + platform_name + "_" + svg_type_name + '_svg = "'
    while len(b) == 1:
        svg_str += "\\" + hex(ord(b))[1:]
        b = svgf.read(1)
    ...
    with open(output_generated_header_filepath, "w") as svgw:
        svgw.write(svg_str)
