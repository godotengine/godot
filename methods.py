...
def generate_platform_export_icons():

    for pngfile in glob.glob("editor/export/*.png"):
        pngf = open(pngfile, "rb")
        b = pngf.read(1)
        str = "/* AUTOGENERATED FILE, DO NOT EDIT */ \n"
        str += "static const unsigned char _" + os.path.basename(pngfile)[:-4] + "[]={"
        while len(b) == 1:
            str += hex(ord(b))
            b = pngf.read(1)
            if len(b) == 1:
        ...
        # NOTE: It is safe to generate this file here, since this is still executed serially
        headerfile = pngfile[:-4] + ".gen.h"
        with open(headerfile, "w") as pngw:
            pngw.write(str)
