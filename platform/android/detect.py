...
def get_opts():
    ...
    return [
        ...
        (
            "ndk_platform",
            'Target platform (android-<api>, e.g. "android-18")',
            "android-18",
        ),
        EnumVariable(
            "android_arch",
...
def configure(env):
    ...
    compiler_path = (
        env["ANDROID_NDK_ROOT"] + "/toolchains/llvm/prebuilt/" + host_subpath + "/bin"
    )
    gcc_toolchain_path = (
    ...
    if env["android_stl"]:
        env.Append(
            CPPFLAGS=[
                "-isystem",
                env["ANDROID_NDK_ROOT"] + "/sources/cxx-stl/llvm-libc++/include",
            ]
        )
        env.Append(
            CPPFLAGS=[
                "-isystem",
                env["ANDROID_NDK_ROOT"] + "/sources/cxx-stl/llvm-libc++abi/include",
            ]
        )
    ...
    if ndk_version != None and LooseVersion(ndk_version) >= LooseVersion(
        "15.0.4075724"
    ):
        ...
        env.Append(
            CPPFLAGS=[
                "-isystem",
                env["ANDROID_NDK_ROOT"] + "/sources/android/support/include",
            ]
        )
        # For unified headers this define has to be set manually
        env.Append(
            CPPFLAGS=["-D__ANDROID_API__=" + str(get_platform(env["ndk_platform"]))]
        )
    else:
    ...
    env.Append(
        CPPFLAGS="-fpic -ffunction-sections -funwind-tables -fstack-protector-strong -fvisibility=hidden -fno-strict-aliasing".split()
    )
    ...
    if env["android_arch"] == "x86":
        ...
        env.Append(
            CPPFLAGS="-D__ARM_ARCH_6__ -march=armv6 -mfpu=vfp -mfloat-abi=softfp".split()
        )

    elif env["android_arch"] == "armv7":
        target_opts = ["-target", "armv7-none-linux-androideabi"]
        env.Append(
            CPPFLAGS="-D__ARM_ARCH_7__ -D__ARM_ARCH_7A__ -march=armv7-a -mfloat-abi=softfp".split()
        )
        if env["android_neon"]:
    ...
    env.Append(
        LINKFLAGS="-Wl,--no-undefined -Wl,-z,noexecstack -Wl,-z,relro -Wl,-z,now".split()
    )
