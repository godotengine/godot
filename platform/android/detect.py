...
def get_opts():
    ...
    return [("ANDROID_NDK_ROOT", "Path to the Android NDK", os.environ.get("ANDROID_NDK_ROOT", 0)), ("ndk_platform", 'Target platform (android-<api>, e.g. "android-18")', "android-18"), EnumVariable("android_arch", "Target architecture", "armv7", ("armv7", "armv6", "arm64v8", "x86")), BoolVariable("android_neon", "Enable NEON support (armv7 only)", True), BoolVariable("android_stl", "Enable Android STL support (for modules)", True)]
...
def configure(env):
    ...
    if env["target"].startswith("release"):
        ...
        env.Append(CPPFLAGS=["-O0", "-D_DEBUG", "-UNDEBUG", "-DDEBUG_ENABLED", "-DDEBUG_MEMORY_ENABLED", "-g", "-fno-limit-debug-info"])
    ...
    env.Append(LIBPATH=[env["ANDROID_NDK_ROOT"] + "/toolchains/" + target_subpath + "/prebuilt/" + host_subpath + "/lib/gcc/" + abi_subpath + "/4.9.x"])
    env.Append(LIBPATH=[env["ANDROID_NDK_ROOT"] + "/toolchains/" + target_subpath + "/prebuilt/" + host_subpath + "/" + abi_subpath + "/lib"])
