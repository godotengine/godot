...
def configure(env: "SConsEnvironment"):
    ...
    if env["accesskit"]:
        if env["simulator"]:
            ak_lib = os.path.join(
                env["accesskit_sdk_path"], "lib", "ios-simulator", "arm64", "static", "libaccesskit.a"
            )
        else:
            ak_lib = os.path.join(env["accesskit_sdk_path"], "lib", "ios", "arm64", "static", "libaccesskit.a")
        if os.path.isfile(ak_lib):
