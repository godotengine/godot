...
def get_opts():
    ...
    return [
        ...
        BoolVariable(["use_mingw", "mingw"], "Use the Mingw compiler, even if MSVC is installed.", False),
        BoolVariable(["use_llvm", "llvm"], "Use the LLVM compiler", False),
        BoolVariable(["use_static_cpp", "static_cpp"], "Link MinGW/MSVC C++ runtime libraries statically", True),
        BoolVariable(["use_asan", "asan"], "Use address sanitizer (ASAN)", False),
        BoolVariable(["use_ubsan", "ubsan"], "Use LLVM compiler undefined behavior sanitizer (UBSAN)", False),
        ...
        BoolVariable(["use_pix", "pix"], "Use PIX (Performance tuning and debugging for DirectX 12) runtime", False),
        (
            "pix_path",
