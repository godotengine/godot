...
def configure(env):
    ...
    if not os.path.exists(em_config_file):
        raise RuntimeError(
            "Emscripten configuration file '%s' does not exist" % em_config_file
        )
    with open(em_config_file) as f:
        ...
        try:
            ...
            raise RuntimeError(
                "Emscripten configuration file '%s' is invalid:\n%s"
                % (em_config_file, e)
            )
    if "EMSCRIPTEN_ROOT" not in em_config:
        raise RuntimeError(
            "'EMSCRIPTEN_ROOT' missing in Emscripten configuration file '%s'"
            % em_config_file
        )
