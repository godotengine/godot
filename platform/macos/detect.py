...
def get_opts():
    ...
    build_deps_folder = os.path.join(str(os.getenv("HOME")), "Library", "Application Support")
    if os.path.exists(build_deps_folder):
...
def configure(env: "SConsEnvironment"):
    ...
    if env["opengl3"]:
        ...
        if env["angle_libs"] != "":
            ...
            if os.path.exists(env["angle_libs"] + "-" + env["arch"] + "-llvm"):
                angle_libs = env["angle_libs"] + "-" + env["arch"] + "-llvm"
            env.Append(LINKFLAGS=["-L" + angle_libs])
