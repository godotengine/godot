...
def get_opts():
    ...
    return [
        ...
        (("APPLE_SDK_PATH", "TVOS_SDK_PATH"), "Path to the tvOS SDK", ""),
        ...
        BoolVariable("generate_bundle", "Generate an APP bundle after building tvOS/macOS binaries", False),
    ]
...
def configure(env: "SConsEnvironment"):
    ...
    if ccache_path is None:
        ...
        # there aren't any ccache wrappers available for tvOS,
    ...
    if env["simulator"]:
        env["APPLE_PLATFORM"] = "tvosimulator"
        env.Append(ASFLAGS=["-mtvos-simulator-version-min=26.0"])
        env.Append(CCFLAGS=["-mtvos-simulator-version-min=26.0"])
        env.Append(CPPDEFINES=["TVOS_SIMULATOR"])
        env.extra_suffix = ".simulator" + env.extra_suffix
    else:
        env["APPLE_PLATFORM"] = "tvos"
        env.Append(ASFLAGS=["-mtvos-version-min=26.0"])
        env.Append(CCFLAGS=["-mtvos-version-min=26.0"])
    ...
    # Temp fix for ABS/MAX/MIN macros in tvOS SDK blocking compilation
    ...
    env.Append(CPPDEFINES=["TVOS_ENABLED", "APPLE_EMBEDDED_ENABLED", "UNIX_ENABLED", "COREAUDIO_ENABLED"])
