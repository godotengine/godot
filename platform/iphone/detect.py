...
def get_opts():
    ...
    return [("IPHONEPLATFORM", "Name of the iPhone platform", "iPhoneOS"), ("IPHONEPATH", "Path to iPhone toolchain", "/Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain"), ("IPHONESDK", "Path to the iPhone SDK", "/Applications/Xcode.app/Contents/Developer/Platforms/${IPHONEPLATFORM}.platform/Developer/SDKs/${IPHONEPLATFORM}.sdk/"), BoolVariable("game_center", "Support for game center", True), BoolVariable("store_kit", "Support for in-app store", True), BoolVariable("icloud", "Support for iCloud", True), BoolVariable("ios_exceptions", "Enable exceptions", False), ("ios_triple", "Triple for ios toolchain", ""), BoolVariable("ios_sim", "Build simulator binary", False)]
...
def configure(env):
    ...
    if env["arch"] == "x86" or env["arch"] == "x86_64":
        arch_flag = "i386" if env["arch"] == "x86" else env["arch"]
        env.Append(LINKFLAGS=["-arch", arch_flag, "-mios-simulator-version-min=9.0", "-isysroot", "$IPHONESDK", "-Xlinker", "-objc_abi_version", "-Xlinker", "2", "-F$IPHONESDK"])
    elif env["arch"] == "arm":
    ...
    env.Append(LINKFLAGS=["-isysroot", "$IPHONESDK", "-framework", "AudioToolbox", "-framework", "AVFoundation", "-framework", "CoreAudio", "-framework", "CoreGraphics", "-framework", "CoreMedia", "-framework", "CoreMotion", "-framework", "Foundation", "-framework", "GameController", "-framework", "MediaPlayer", "-framework", "OpenGLES", "-framework", "QuartzCore", "-framework", "Security", "-framework", "SystemConfiguration", "-framework", "UIKit"])
