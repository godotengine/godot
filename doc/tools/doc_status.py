...
flags = {"c": platform.platform() != "Windows", "b": False, "g": False, "s": False, "u": False, "h": False, "p": False, "o": True, "i": False, "a": True, "e": False}  # Disable by default on windows, since we use ANSI escape codes
flag_descriptions = {"c": "Toggle colors when outputting.", "b": "Toggle showing only not fully described classes.", "g": "Toggle showing only completed classes.", "s": "Toggle showing comments about the status.", "u": "Toggle URLs to docs.", "h": "Show help and exit.", "p": "Toggle showing percentage as well as counts.", "o": "Toggle overall column.", "i": "Toggle collapse of class items columns.", "a": "Toggle showing all items.", "e": "Toggle hiding empty items."}
long_flags = {"colors": "c", "use-colors": "c", "bad": "b", "only-bad": "b", "good": "g", "only-good": "g", "comments": "s", "status": "s", "urls": "u", "gen-url": "u", "help": "h", "percent": "p", "use-percentages": "p", "overall": "o", "use-overall": "o", "items": "i", "collapse": "i", "all": "a", "empty": "e"}
...
colors = {"name": [36], "part_big_problem": [4, 31], "part_problem": [31], "part_mostly_good": [33], "part_good": [32], "url": [4, 34], "section": [1, 4], "state_off": [36], "state_on": [1, 35]}  # cyan  # underline, red  # red  # yellow  # green  # underline, blue  # bold, underline  # cyan  # bold, magenta/plum
...
class ClassStatus:
    def __init__(self, name=""):
        ...
        self.progresses = {"methods": ClassStatusProgress(), "constants": ClassStatusProgress(), "members": ClassStatusProgress(), "signals": ClassStatusProgress()}
    ...
    def make_output(self):
        ...
        description_progress = ClassStatusProgress((self.has_brief_description + self.has_description) * overall_progress_description_weigth, 2 * overall_progress_description_weigth)
...
if len(input_file_list) < 1 or flags["h"]:
    ...
    for flag in flag_list:
        ...
        print(("{synonyms} (Currently " + color("state_" + ("on" if flags[flag] else "off"), "{value}") + ")\n\t{description}").format(synonyms=", ".join(synonyms), value=("on" if flags[flag] else "off"), description=flag_descriptions[flag]))
    sys.exit(0)
