...
godot_docs_pattern = re.compile(
    "^http(?:s)?:\/\/docs\.godotengine\.org\/(?:[a-zA-Z0-9\.\-_]*)\/(?:[a-zA-Z0-9\.\-_]*)\/(.*)\.html(#.*)?$"
)
...
if len(input_list) < 1:
    print(
        "usage: makerst.py <path to folders> and/or <path to .xml files> (order of arguments irrelevant)"
    )
    ...
    while True:
        ...
        if tag_text in class_names:
            ...
            if cmd == "/codeblock":
                ...
                cmd.find("method") == 0
                or cmd.find("member") == 0
                or cmd.find("signal") == 0
            ):
                cmd = tag_text[:space_pos]
                param = tag_text[space_pos + 1 :]
                ...
                if param.find(".") != -1:
                    ...
                    tag_text = (
                        ":ref:`" + param + "<class_" + cclass + "_" + param + ">`"
                    )
                escape_post = True
        ...
        if (
            escape_post and post_text and post_text[0].isalnum()
...
def make_method(f, cname, method_data, declare, event=False, pp=None):
...
def make_properties(f, cname, prop_data, description=False, pp=None):
    t = ""
    ...
    if description:
        ...
        setget = []
        if (
            "setter" in prop_data.attrib
            and prop_data.attrib["setter"] != ""
            and not prop_data.attrib["setter"].startswith("_")
        ):
            setget.append(("*Setter*", prop_data.attrib["setter"] + "(value)"))
        if (
            "getter" in prop_data.attrib
            and prop_data.attrib["getter"] != ""
            and not prop_data.attrib["getter"].startswith("_")
        ):
            setget.append(("*Getter*", prop_data.attrib["getter"] + "()"))
    else:
        s = (
            ":ref:`"
            + prop_data.attrib["name"]
            + "<class_"
            + cname
            + "_"
            + prop_data.attrib["name"]
