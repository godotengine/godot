...
TwoBodyConstraint *DistanceConstraintSettings::Create(Body &inBody1, Body &inBody2) const {
	return new DistanceJoint(inBody1, inBody2, *this);
}

DistanceJoint::DistanceJoint(Body &inBody1, Body &inBody2, const DistanceConstraintSettings &inSettings) :
...
void DistanceJoint::NotifyShapeChanged(const BodyID &inBodyID, Vec3Arg inDeltaCOM) {
...
void DistanceJoint::CalculateConstraintProperties(float inDeltaTime) {
...
void DistanceJoint::SetupVelocityConstraint(float inDeltaTime) {
	CalculateConstraintProperties(inDeltaTime);
}

void DistanceJoint::ResetWarmStart() {
	mAxisConstraint.Deactivate();
}

void DistanceJoint::WarmStartVelocityConstraint(float inWarmStartImpulseRatio) {
	mAxisConstraint.WarmStart(*mBody1, *mBody2, mWorldSpaceNormal, inWarmStartImpulseRatio);
}

bool DistanceJoint::SolveVelocityConstraint(float inDeltaTime) {
...
bool DistanceJoint::SolvePositionConstraint(float inDeltaTime, float inBaumgarte) {
...
#ifdef JPH_DEBUG_RENDERER
void DistanceJoint::DrawConstraint(DebugRenderer *inRenderer) const {
	// Draw constraint
	Vec3 delta = Vec3(mWorldSpacePosition2 - mWorldSpacePosition1);
...
void DistanceJoint::SaveState(StateRecorder &inStream) const {
...
void DistanceJoint::RestoreState(StateRecorder &inStream) {
...
Ref<ConstraintSettings> DistanceJoint::GetConstraintSettings() const {
