...
{
	return SVec3(v1.x + v2.x, v1.y + v2.y, v1.z + v2.z);
}
...
{
	return SVec3( fS * v.x, fS * v.y, fS * v.z);
}
...
static int GenerateInitialVerticesIndexList(STriInfo pTriInfos[], int piTriList_out[], const ScppMikkTSpaceContext * pContext, const int iNrTrianglesIn);
static void GenerateSharedVerticesIndexList(int piTriList_in_and_out[], const ScppMikkTSpaceContext * pContext, const int iNrTrianglesIn);
static void InitTriInfo(STriInfo pTriInfos[], const int piTriListIn[], const ScppMikkTSpaceContext * pContext, const int iNrTrianglesIn);
...
static tbool GenerateTSpaces(STSpace psTspace[], const STriInfo pTriInfos[], const SGroup pGroups[],
                             ...
                             const ScppMikkTSpaceContext * pContext);
...
static SVec3 GetPosition(const ScppMikkTSpaceContext * pContext, const int index);
static SVec3 GetNormal(const ScppMikkTSpaceContext * pContext, const int index);
static SVec3 GetTexCoord(const ScppMikkTSpaceContext * pContext, const int index);


// degen triangles
static void DegenPrologue(STriInfo pTriInfos[], int piTriList_out[], const int iNrTrianglesIn, const int iTotTris);
static void DegenEpilogue(STSpace psTspace[], STriInfo pTriInfos[], int piTriListIn[], const ScppMikkTSpaceContext * pContext, const int iNrTrianglesIn, const int iTotTris);


tbool genTangSpaceDefault(const ScppMikkTSpaceContext * pContext)
{
...
static void MergeVertsFast(int piTriList_in_and_out[], STmpVert pTmpVert[], const ScppMikkTSpaceContext * pContext, const int iL_in, const int iR_in);
static void MergeVertsSlow(int piTriList_in_and_out[], const ScppMikkTSpaceContext * pContext, const int pTable[], const int iEntries);
static void GenerateSharedVerticesIndexListSlow(int piTriList_in_and_out[], const ScppMikkTSpaceContext * pContext, const int iNrTrianglesIn);
	...
	for (f=0; f<pContext->m_pInterface->m_getNumFaces(pContext); f++)
	{
		...
		else
		{
...

			{
				...
				//todo: convert the rest of these to match distSQ_02
				...
				const float distSQ_02 = LengthSquared(vsub(T2, GetTexCoord(pContext, i0)));
...
static float CalcTexArea(const ScppMikkTSpaceContext * pContext, const int indices[])
{
...
static STSpace EvalTspace(int face_indices[], const int iFaces, const int piTriListIn[], const STriInfo pTriInfos[], const ScppMikkTSpaceContext * pContext, const int iVertexRepresentitive);
...
static STSpace EvalTspace(int face_indices[], const int iFaces, const int piTriListIn[], const STriInfo pTriInfos[],
                          const ScppMikkTSpaceContext * pContext, const int iVertexRepresentitive)
{
	STSpace res;
	float fAngleSum = 0;
	int face=0;
...
static void BuildNeighborsFast(STriInfo pTriInfos[], SEdge * pEdges, const int piTriListIn[], const int iNrTrianglesIn)
{
	...
	for (f=0; f<iNrTrianglesIn; f++)
		for (i=0; i<3; i++)
		{
			//todo: unroll this loop. 2 ints made and 3 branches (ternary)
...
static void QuickSortEdges(SEdge * pSortBuffer, int iLeft, int iRight, const int channel, unsigned int uSeed)
{
	...
	// Random
	//todo: Why is this here for a sort? I can't imagine.
	t=uSeed&31;
	t=(uSeed<<t)|(uSeed>>(32-t));
	uSeed=uSeed+t+3;
	// Random end
	...
	do
	{
		...
		if (iL <= iR)
		{
			//todo: overload ^= operator for quick swap? use std::swap?
			sTmp = pSortBuffer[iL];
			pSortBuffer[iL] = pSortBuffer[iR];
			pSortBuffer[iR] = sTmp;
			++iL; --iR;
