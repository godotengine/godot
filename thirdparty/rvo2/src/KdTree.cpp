...
namespace RVO {
	KdTree::KdTree() : agents_(NULL), obstacleTree_(NULL) { }
	...
	void KdTree::buildObstacleTree(std::vector<Obstacle *> obstacles)
	{
		deleteObstacleTree(obstacleTree_);
	...
	KdTree::ObstacleTreeNode *KdTree::buildObstacleTreeRecursive(const std::vector<Obstacle *> &obstacles, std::vector<Obstacle *> root)
	{
		if (obstacles.empty()) {
			...
			node->left = buildObstacleTreeRecursive(leftObstacles, root);
			node->right = buildObstacleTreeRecursive(rightObstacles, root);
			return node;
		}
	}
