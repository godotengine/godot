...
EM_JS(int, SDL_GetEmscriptenJoystickVendor, (int gamepad_idx), {
	let gamepads = navigator['getGamepads']();
	if (!gamepads) {
		return 0;
	}
	let gamepad = gamepads[gamepad_idx];
    ...
    if (id_split['length'] > 1 && !isNaN(parseInt(id_split[0]))) {
...
EM_JS(int, SDL_GetEmscriptenJoystickProduct, (int gamepad_idx), {
	let gamepads = navigator['getGamepads']();
	if (!gamepads) {
		return 0;
	}
    ...
    // Firefox, Safari: 046d-c216-Logitech Dual Action (or 46d-c216-Logicool Dual Action)
    let id_split = gamepad['id']['split']('-');
    if (id_split['length'] > 1 && !isNaN(parseInt(id_split[1]))) {
