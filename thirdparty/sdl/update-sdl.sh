...
rm -rf atomic audio core cpuinfo events haptic hidapi include io joystick libm loadso \
  sensor stdlib thread timer video *.c *.h CREDITS.md LICENSE.txt
rm -rf *.tar.gz tmp
...
mkdir $target/audio
cp -rv audio/{*.{c,h},alsa,coreaudio,pulseaudio,openslES,wasapi} $target/audio
...
mkdir -p $target/video/android
cp -rv video/android/SDL_androidevents.{c,h} $target/video/android
...
for dir in hidapi linux mac windows; do
  ...
  cp -v hidapi/$dir/*.{c,h} $target/hidapi/$dir || true
done
