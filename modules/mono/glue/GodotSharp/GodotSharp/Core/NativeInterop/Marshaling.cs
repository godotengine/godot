...
namespace Godot.NativeInterop
{
    ...
    internal static class Marshaling
    {
        ...
        /* TODO: Reflection each time is slow. This will be replaced with source generators.
         However we still need this as a fallback, specially for other .NET languages.
         Now that the code is in C#, we can use Reflection.Emit instead. Later it may be
         possible to do the same with: https://github.com/dotnet/runtime/issues/45152 */
        ...
        private static unsafe godot_variant mono_object_to_variant_impl(object p_obj, bool p_fail_with_err = true)
        {
            ...
            switch (p_obj)
            {
                ...
                case Quat @quat:
                    return VariantUtils.CreateFromQuat(@quat);
                case Transform @transform:
                    return VariantUtils.CreateFromTransform(@transform);
                case AABB @aabb:
        ...
        public static unsafe object variant_to_mono_object_of_type(godot_variant* p_var, Type type)
        {
            ...
            switch (Type.GetTypeCode(type))
            {
                ...
                default:
                {
                    ...
                    if (type == typeof(Quat))
                        return VariantUtils.ConvertToQuat(p_var);

                    if (type == typeof(Transform))
                        return VariantUtils.ConvertToTransform(p_var);
        ...
        public static godot_callable ConvertCallableToNative(ref Callable p_managed_callable)
        {
            ...
            else
            {
                ...
                unsafe
                {
                    godot_string_name method = NativeFuncs.godotsharp_string_name_new_copy(&src);
        ...
        public static godot_signal ConvertSignalToNative(ref SignalInfo p_managed_signal)
        {
            ...
            unsafe
            {
                ...
                {
                    _name = NativeFuncs.godotsharp_string_name_new_copy(&src),
                    _objectId = ownerId
                };
            }
        }
