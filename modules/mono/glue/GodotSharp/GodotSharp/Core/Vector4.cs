...
namespace Godot
{
    ...
    public struct Vector4 : IEquatable<Vector4>
    {
        ...
        /// <c>[2]</c> is equivalent to <see cref="w"/>.
        /// </value>
        ...
        public Vector4 Bounce(Vector4 normal)
        {
        ...
        {
            return new Vector4(to.x - x, to.y - y, to.z - z,to.w - w).Normalized();
        }
        ...
        /// </summary>
        /// <seealso cref="DistanceSquaredTo(Vector4)"/>
        /// <param name="to">The other vector to use.</param>
        ...
        public Vector4 LimitLength(real_t length = 1.0f)
        {
            Vector4 v = this;
            real_t l = Length();
        ...
        {
            if (x > y) {
                return (x > w) ? ((x > z) ? Axis.X : Axis.Z) : ((z > w) ? Axis.Z : Axis.W);
            } else {
                return (y > w) ? ((y > z) ? Axis.Y : Axis.Z) : ((z > w) ? Axis.Z : Axis.W);
            }
        }
        ...
        {
            if (z < y) {
                return (z < x) ? ((z < w) ? Axis.Z : Axis.W) : ((w < x) ? Axis.W : Axis.X);
            } else {
                return (y < x) ? ((y < w) ? Axis.Y : Axis.W) : ((w < x) ? Axis.W : Axis.X);
            }
        }
        ...
        public Vector4 MoveToward(Vector4 to, real_t delta)
        {
            Vector4 v = this;
            Vector4 vd = to - v;
        ...
        /// </summary>
        /// <value>Equivalent to <c>new Vector4(Mathf.Inf, Mathf.Inf, Mathf.Inf)</c>.</value>
        ...
        /// This is the same as writing <c>new Vector4(-v.x, -v.y, -v.z)</c>.
        ...
        /// with the Y values of the two vectors, then with the Z values and then with the W values.
