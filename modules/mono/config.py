...
def configure(env):
    ...
    envvars.Add(
        PathVariable(
            "mono_assemblies_output_dir",
            "Path to the assemblies output directory",
            "#bin",
    ...
    if env["platform"] == "windows":
        ...
        if mono_static:
            ...
            mono_lib_name = find_file_in_dir(
                mono_lib_path, mono_lib_names, extension=".lib"
            )
            ...
            mono_dll_name = find_file_in_dir(
                mono_bin_path, mono_lib_names, extension=".dll"
            )
        ...
        copy_file(
            os.path.join(mono_lib_path, "mono", "4.5"),
            assemblies_output_dir,
            "mscorlib.dll",
        )
    else:
        ...
        if mono_root:
            ...
            mono_lib = find_file_in_dir(
                mono_lib_path, mono_lib_names, prefix="lib", extension=".a"
            )
            ...
            if not mono_static:
                mono_so_name = find_file_in_dir(
                    mono_lib_path, mono_lib_names, prefix="lib", extension=sharedlib_ext
                )
            ...
            for hint_dir in tmpenv["LIBPATH"]:
                name_found = find_file_in_dir(
                    hint_dir, mono_lib_names, prefix="lib", extension=sharedlib_ext
                )
                if name_found:
            ...
            copy_file(mono_lib_path, "bin", "lib" + mono_so_name + sharedlib_ext)
            copy_file(
                os.path.join(mono_prefix, "lib", "mono", "4.5"),
                assemblies_output_dir,
                "mscorlib.dll",
            )
...
def pkgconfig_try_find_mono_version():
    ...
    lines = subprocess.check_output(
        ["pkg-config", "monosgen-2", "--modversion"]
    ).splitlines()
...
def mono_root_try_find_mono_version(mono_root):
    ...
    try:
        return LooseVersion(
            first_line.split()[len("Mono JIT compiler version".split())]
        )
