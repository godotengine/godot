    ...
    public static class DotNetFinder
    {
        public static string? FindDotNetExe()
        {
            ...
            if (OS.IsMacOS)
            {
                ...
                if (RuntimeInformation.OSArchitecture == Architecture.X64)
                {
                    string dotnet_hb = "/usr/local/opt/dotnet/libexec"; // Look for Intel homebrew version.
                ...
                {
                    string dotnet_hb = "/opt/homebrew/opt/dotnet/libexec"; // Look for ARM homebrew version.
                    if (File.Exists(dotnet_hb))
                    {
                        return dotnet_hb;
                    }
