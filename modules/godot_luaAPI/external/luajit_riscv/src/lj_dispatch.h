...
typedef struct GG_State {
  ...
  /* 通过 K12 编码的 DISPATCH 相对寻址使 g 可访问。*/
  uint8_t align1[(16 - sizeof(global_State)) & 15];
#endif
#if LJ_TARGET_MIPS
  ASMFunction got[LJ_GOT__MAX];          /* 全局偏移表。*/
#endif
#if LJ_HASJIT
  jit_State J;                           /* JIT 状态。*/
  HotCount hotcount[HOTCOUNT_SIZE];      /* 热点计数器。*/
#if LJ_TARGET_ARM && !LJ_TARGET_NX
  /* J 的同样情况。*/
  uint8_t align2[(16 - sizeof(jit_State) - sizeof(HotCount) * HOTCOUNT_SIZE) & 15];
#endif
#endif
  ASMFunction dispatch[GG_LEN_DISP];     /* 指令分发表。*/
  BCIns bcff[GG_NUM_ASMFF];              /* ASM 快速函数的字节码。*/
} GG_State;
