...
{
  lj_buf_reset(&ls->sb);  // 重置字符串缓冲区

  for (;;) {
    if (lj_char_isident(ls->c)) {
      GCstr *s;
      if (lj_char_isdigit(ls->c)) {  // 数字字面量。
        lex_number(ls, tv);
        return TK_number;
      }
      // 标识符或保留字。
    ...
    case '-':
      ...
      if (ls->c == '[') {  // 长注释 "--[=*[...]=*]"。
        ...
        lj_buf_reset(&ls->sb);  // `lex_skipeq' 可能会使缓冲区脏。
        if (sep >= 0) {
