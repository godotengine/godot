...
/* Upvalue 引用实际上是加载操作，但没有对应的存储操作。
** 因此，对它们进行公共子表达式消除 (CSE) 是可以的，但跨 GC 步骤的受保护的 UREFO 除外。
** 如果引用的函数是常数，则其 Upvalue 地址也是常数。
** 这可以用于通过查找相同的地址来提高 CSE，
** 即使 Upvalue 来自于不同的函数也可以这样做。
*/
LJFOLD(UREFO KGC any)
LJFOLD(UREFC KGC any)
LJFOLDF(cse_uref)
{
  // 如果启用了 CSE，则尝试在同一链上查找相同的地址。
  if (LJ_LIKELY(J->flags & JIT_F_OPT_CSE)) {
...
TRef LJ_FASTCALL lj_opt_fold(jit_State *J)
{
  ...
  // 如果折叠被禁用或者当前 IR 指令类型不支持折叠，则跳转到 CSE。
...
retry:
  ...
  if (fins->op2 >= J->cur.nk) {
    ...
    *fright = *IR(fins->op2);
    ...
    key += (fins->op2 & 0x3ffu);  // 字面值掩码。必须包括 IRCONV_*MASK。
  }
...
/* 对IR指令进行公共子表达式消除（CSE）。由于跳表链的使用，这是非常快速的。 */
TRef LJ_FASTCALL lj_opt_cse(jit_State *J)
{
  ...
  if (LJ_LIKELY(J->flags & JIT_F_OPT_CSE)) {
    ...
    IRRef lim = fins->op1;
    if (fins->op2 > lim) lim = fins->op2;  /* 依赖于 lit < REF_BIAS。 */
    while (ref > lim) {
      if (IR(ref)->op12 == op12)
        return TREF(ref, irt_t(IR(ref)->t));  /* 找到共同子表达式。 */
      ref = IR(ref)->prev;
    }
  }
  /* 否则发出IR（为了速度而内联）。 */
