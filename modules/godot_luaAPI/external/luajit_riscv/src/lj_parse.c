...
typedef struct FuncState {
  ...
  lua_State *L;               /* Lua 状态。 */
  FuncScope *bl;              /* 当前作用域。 */
  struct FuncState *prev;     /* 包围该函数的函数。 */
  BCPos pc;                   /* 下一个字节码位置。 */
  BCPos lasttarget;           /* 最后跳转目标的字节码位置。 */
  BCPos jpc;                  /* 下一个字节码的等待跳转列表。 */
  BCReg freereg;              /* 第一个空闲寄存器。 */
  BCReg nactvar;              /* 活跃局部变量的数量。 */
  BCReg nkn, nkgc;            /* lua_Number/GCobj 常量的数量。 */
  BCLine linedefined;         /* 函数定义的第一行。 */
  BCInsLine *bcbase;          /* 字节码栈的基地址。 */
  BCPos bclim;                /* 字节码栈的限制。 */
  MSize vbase;                /* 该函数的变量栈的基地址。 */
  uint8_t flags;              /* 原型标志。 */
  uint8_t numparams;          /* 参数的数量。 */
  uint8_t framesize;          /* 固定栈帧大小。 */
  uint8_t nuv;                /* upvalue 的数量。 */
  VarIndex varmap[LJ_MAX_LOCVAR];  /* 从寄存器到变量索引的映射。 */
  VarIndex uvmap[LJ_MAX_UPVAL];    /* 从 upvalue 到变量索引的映射。 */
  VarIndex uvtmp[LJ_MAX_UPVAL];   /* 临时 upvalue 映射。 */
} FuncState;
...
static void parse_body(LexState *ls, ExpDesc *e, int needself, BCLine line)
{
  ...
  pfs->bcbase = ls->bcstack + oldbase;  /* 可能已重新分配。 */
...
static void parse_func(LexState *ls, BCLine line)
{
  ...
  ExpDesc v, b;
  int needself = 0;
  lj_lex_next(ls);  /* 跳过 'function'。 */
  /* 解析函数名。 */
  var_lookup(ls, &v);
  while (ls->tok == '.')  /* 多个点分隔的字段。 */
    expr_field(ls, &v);
  if (ls->tok == ':') {  /* 可选的冒号表示方法调用。 */
    needself = 1;
    expr_field(ls, &v);
  }
  ...
  fs = ls->fs;
  bcemit_store(fs, &v, &b);
  fs->bcbase[fs->pc - 1].line = line;  /* 为存储设置行号。 */
}
...
static void parse_return(LexState *ls)
{
  BCIns ins;
  FuncState *fs = ls->fs;
  lj_lex_next(ls);  /* 跳过 'return'。*/
  fs->flags |= PROTO_HAS_RETURN;
  
  if (parse_isend(ls->tok) || ls->tok == ';') {  /* 无返回值的 return。 */
    ins = BCINS_AD(BC_RET0, 0, 1);
  } else {  /* 返回一个或多个值。 */
    ExpDesc e;  /* 保存列表中的 _最后一个_ 表达式。 */
    ...
    if (nret == 1) {  /* 返回一个结果。 */
      ...
      if (e.k == VCALL) {  /* 从调用中追加所有结果。*/
      notailcall:
        setbc_b(bcptr(fs, &e), 0);
        ins = BCINS_AD(BC_RETM, fs->nactvar, e.u.s.aux - fs->nactvar);
      } else {
        expr_tonextreg(fs, &e);  /* 强制连续的寄存器。 */
  ...
  if (fs->flags & PROTO_CHILD)
    bcemit_AJ(fs, BC_UCLO, 0, 0);  /* 可能需要先关闭 upvalues。*/
  bcemit_INS(fs, ins);
}
...
static void parse_for_iter(LexState *ls, GCstr *indexname)
{
  ...
  /* 迭代器需要另外 3 [4] 个插槽（func [pc] | state ctl）。*/
  bcreg_bump(fs, 3+LJ_FR2);
  isnext = (nvars <= 5 && predict_next(ls, fs, exprpc));
  var_add(ls, 3);  /* 隐藏的控制变量。 */
  lex_check(ls, TK_do);
  loop = bcemit_AJ(fs, isnext ? BC_ISNEXT : BC_JMP, base, NO_JMP);
  fscope_begin(fs, &bl, 0);  /* 可见变量的作用域。 */
  var_add(ls, nvars-3);
  ...
  fs->bcbase[loopend-1].line = line;  /* 修复控制指令的行数。 */
...
static int parse_stmt(LexState *ls)
{
  ...
  switch (ls->tok) {
    ...
    case TK_break:
      lj_lex_next(ls);
      parse_break(ls);
      return !LJ_52;  /* 在 Lua 5.1 中必须是最后一个。 */
  #if LJ_52
    case ';':
      lj_lex_next(ls);
      break;
...
static void parse_chunk(LexState *ls)
{
  ...
  while (!islast && !parse_isend(ls->tok)) {
    ...
    lj_assertLS(ls->fs->framesize >= ls->fs->freereg &&
		...
		"bad regalloc");
    ls->fs->freereg = ls->fs->nactvar;  /* 每个语句后释放寄存器。 */
  }
  synlevel_end(ls);
}
...
GCproto *lj_parse(LexState *ls)
{
  ...
  FuncScope bl;        // 函数作用域
  GCproto *pt;         // 解析后的函数原型
  lua_State *L = ls->L; // Lua 状态
#ifdef LUAJIT_DISABLE_DEBUGINFO
  ls->chunkname = lj_str_newlit(L, "=");  // 默认使用 "=" 作为 chunkname
#else
  ls->chunkname = lj_str_newz(L, ls->chunkarg);  // 使用给定的 chunkarg 作为 chunkname
#endif
  setstrV(L, L->top, ls->chunkname);  // 将 chunkname 字符串放入栈顶，作为锚点字符串
  incr_top(L);
  ls->level = 0;  // 初始化解析器层级
  fs_init(ls, &fs);  // 初始化函数状态
  fs.linedefined = 0;  // 起始行号为 0
  fs.numparams = 0;    // 参数个数为 0
  fs.bcbase = NULL;    // 字节码数组为空
  fs.bclim = 0;        // 字节码数组长度为 0
  fs.flags |= PROTO_VARARG;  // 主 chunk 总是一个可变参数函数
  fscope_begin(&fs, &bl, 0);  // 开始函数作用域
  bcemit_AD(&fs, BC_FUNCV, 0, 0);  // 发送 BC_FUNCV 指令，作为占位符
  lj_lex_next(ls);  // 预读第一个 token
  parse_chunk(ls);  // 解析 chunk
  if (ls->tok != TK_eof)
    err_token(ls, TK_eof);  // 如果不是 EOF，报错
  pt = fs_finish(ls, ls->linenumber);  // 完成函数状态，获取函数原型
  L->top--;  // 弹出 chunkname
  lj_assertL(fs.prev == NULL && ls->fs == NULL, "mismatched frame nesting");
  lj_assertL(pt->sizeuv == 0, "toplevel proto has upvalues");  // 顶层 proto 不应该有 Upvalues
  return pt;  // 返回解析后的函数原型
}
