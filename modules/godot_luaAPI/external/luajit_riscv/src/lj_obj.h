...
struct lua_State {
  GCHeader;             /* GC 头部。*/
  uint8_t dummy_ffid;   /* 用于在虚拟帧上的 curr_funcisL() 中的假 FF_C。*/
  uint8_t status;       /* 线程状态。*/
  MRef glref;           /* 全局状态的链接。*/
  GCRef gclist;         /* GC 链。*/
  TValue *base;         /* 当前执行函数的基址。*/
  TValue *top;          /* 栈中第一个空闲槽位。*/
  MRef maxstack;        /* 栈中的最后一个空闲槽位。*/
  MRef stack;           /* 栈基址。*/
  GCRef openupval;      /* 栈中打开的 upvalues 列表。*/
  GCRef env;            /* 线程环境（全局变量的表）。*/
  void *cframe;         /* C 栈帧链的末尾。*/
  MSize stacksize;      /* 真实栈大小（包括 LJ_STACK_EXTRA）。*/
};
