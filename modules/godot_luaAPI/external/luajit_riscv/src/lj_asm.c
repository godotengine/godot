...
static void asm_ir(ASMState *as, IRIns *ir)
{
  switch ((IROp)ir->o) {
  /* 杂项操作。 */
  case IR_LOOP: asm_loop(as); break;  // 循环头
  case IR_NOP: case IR_XBAR:
    lj_assertA(!ra_used(ir),
	       "IR %04d not unused", (int)(ir - as->ir) - REF_BIAS);
    break;  // 空操作或交叉栈操作
  case IR_USE:
    ra_alloc1(as, ir->op1, irt_isfp(ir->t) ? RSET_FPR : RSET_GPR); break;  // 加载使用的值到寄存器
  case IR_PHI: asm_phi(as, ir); break;  // PHI 操作
  case IR_HIOP: asm_hiop(as, ir); break;  // 高位运算
  case IR_GCSTEP: asm_gcstep(as, ir); break;  // GC 步骤
  case IR_PROF: asm_prof(as, ir); break;  // 效能分析
  ...
  case IR_ABC:
    asm_comp(as, ir);  // 条件比较
    break;
  case IR_EQ: case IR_NE: asm_fuseequal(as, ir); break;  // 相等性比较

  case IR_RETF: asm_retf(as, ir); break;  // 函数返回

  /* 位操作。 */
  case IR_BNOT: asm_bnot(as, ir); break;  // 按位取反
  case IR_BSWAP: asm_bswap(as, ir); break;  // 字节交换
  case IR_BAND: asm_band(as, ir); break;  // 按位与
  case IR_BOR: asm_bor(as, ir); break;  // 按位或
  case IR_BXOR: asm_bxor(as, ir); break;  // 按位异或
  case IR_BSHL: asm_bshl(as, ir); break;  // 左移
  case IR_BSHR: asm_bshr(as, ir); break;  // 算术右移
  case IR_BSAR: asm_bsar(as, ir); break;  // 逻辑右移
  case IR_BROL: asm_brol(as, ir); break;  // 循环左移
  case IR_BROR: asm_bror(as, ir); break;  // 循环右移

  /* 算术操作。 */
  case IR_ADD: asm_add(as, ir); break;  // 加法
  case IR_SUB: asm_sub(as, ir); break;  // 减法
  case IR_MUL: asm_mul(as, ir); break;  // 乘法
  case IR_MOD: asm_mod(as, ir); break;  // 求余
  case IR_NEG: asm_neg(as, ir); break;  // 取负
#if LJ_SOFTFP32
  ...
  case IR_LDEXP: case IR_FPMATH: case IR_TOBIT:
    ...
    break;
#else
  case IR_DIV: asm_div(as, ir); break;  // 除法
  case IR_POW: asm_pow(as, ir); break;  // 幂运算
  case IR_ABS: asm_abs(as, ir); break;  // 绝对值
  case IR_LDEXP: asm_ldexp(as, ir); break;  // 乘方运算
  case IR_FPMATH: asm_fpmath(as, ir); break;  // 浮点数运算
  case IR_TOBIT: asm_tobit(as, ir); break;  // 转换为整数
#endif
  case IR_MIN: asm_min(as, ir); break;  // 取最小值
  case IR_MAX: asm_max(as, ir); break;  // 取最大值

  /* 溢出检查算术操作。 */
  case IR_ADDOV: asm_addov(as, ir); break;  // 加法溢出检查
  case IR_SUBOV: asm_subov(as, ir); break;  // 减法溢出检查
  case IR_MULOV: asm_mulov(as, ir); break;  // 乘法溢出检查

  /* 内存引用。 */
  case IR_AREF: asm_aref(as, ir); break;  // 数组元素引用
  case IR_HREF: asm_href(as, ir, 0); break;  // hash 表元素引用
  case IR_HREFK: asm_hrefk(as, ir); break;  // hash 表常量元素引用
  case IR_NEWREF: asm_newref(as, ir); break;  // 新建引用
  case IR_UREFO: case IR_UREFC: asm_uref(as, ir); break;  // upvalue 引用
  case IR_FREF: asm_fref(as, ir); break;  // 外部函数引用
  case IR_TMPREF: asm_tmpref(as, ir); break;  // 临时引用
  case IR_STRREF: asm_strref(as, ir); break;  // 字符串引用
  case IR_LREF: asm_lref(as, ir); break;  // lightuserdata 引用

  /* 载入和存储。 */
  case IR_ALOAD: case IR_HLOAD: case IR_ULOAD: case IR_VLOAD:
    asm_ahuvload(as, ir);
    break;  // 数组、hash 表、upvalue、vararg 元素加载
  case IR_FLOAD: asm_fload(as, ir); break;  // 外部函数加载
  case IR_XLOAD: asm_xload(as, ir); break;  // lightuserdata 加载
  case IR_SLOAD: asm_sload(as, ir); break;  // 字符串加载
  case IR_ALEN: asm_alen(as, ir); break;  // 数组长度

  case IR_ASTORE: case IR_HSTORE: case IR_USTORE: asm_ahustore(as, ir); break;  // 数组、hash 表、upvalue、vararg 元素存储
  case IR_FSTORE: asm_fstore(as, ir); break;  // 外部函数存储
  case IR_XSTORE: asm_xstore(as, ir); break;  // lightuserdata 存储

  /* 分配。 */
  case IR_SNEW: case IR_XSNEW: asm_snew(as, ir); break;  // 分配新的数组
  case IR_TNEW: asm_tnew(as, ir); break;  // 分配新的 table
  case IR_TDUP: asm_tdup(as, ir); break;  // 复制 table
  case IR_CNEW: case IR_CNEWI:
#if LJ_HASFFI
    asm_cnew(as, ir);  // 分配新的 C 数据
#else
    lj_assertA(0, "IR %04d with unused op %d",
		  (int)(ir - as->ir) - REF_BIAS, ir->o);
#endif
  ...
  /* 缓冲区操作。 */
  case IR_BUFHDR: asm_bufhdr(as, ir); break;  // 初始化缓冲区头
  case IR_BUFPUT: asm_bufput(as, ir); break;  // 向缓冲区添加字符
  case IR_BUFSTR: asm_bufstr(as, ir); break;  // 缓冲区转字符串

  /* 写屏障。 */
  case IR_TBAR: asm_tbar(as, ir); break;  // table 写屏障
  case IR_OBAR: asm_obar(as, ir); break;  // upvalue 或全局变量写屏障

  /* 类型转换。 */
  case IR_CONV: asm_conv(as, ir); break;  // 类型转换
  case IR_TOSTR: asm_tostr(as, ir); break;  // 转为字符串
  case IR_STRTO: asm_strto(as, ir); break;  // 从字符串转换

  /* 调用。 */
  case IR_CALLA:
    as->gcsteps++;
    /* fallthrough */
  case IR_CALLN: case IR_CALLL: case IR_CALLS: asm_call(as, ir); break;  // 函数调用
  case IR_CALLXS: asm_callx(as, ir); break;  // 外部函数调用
  case IR_CARG: break;  // 函数调用参数
...
/* 侧边跟踪的头部。
**
** 当前简化的算法要求从父级继承的所有插槽在第2和第3遍之间都在寄存器中是活跃的。
** 这避免了堆栈插槽重新排序的复杂性。但当然，这在某些情况下可能会溢出寄存器集，
** 并引发可怕的错误："NYI: register coalescing too complex"（尚未实现：寄存器合并过于复杂）。
** 需要一种更精细的算法。
*/
static void asm_head_side(ASMState *as)
{
  IRRef1 sloadins[RID_MAX];
  RegSet allow = RSET_ALL;  /* 所有合并寄存器的补集。 */ /* Inverse of all coalesced registers. */
  RegSet live = RSET_EMPTY; /* 活跃的父级寄存器。 */ /* Live parent registers. */
  RegSet pallow = RSET_GPR; /* 父级堆栈检查需要的寄存器。 */ /* Registers needed by the parent stack check. */
  Reg pbase;
  IRIns *irp = &as->parent->ir[REF_BASE]; /* 父级基址。 */ /* Parent base. */
  ...
  for (i = as->stopins; i > REF_BASE; i--) {
    ...
    if (ir->r == rs) {  /* 立即合并匹配的寄存器。 */ /* Coalesce matching registers right now. */
      ra_free(as, ir->r);
	...
	pass3 = 1;
      ...
      rset_set(live, rs); /* 阻止活跃的父级寄存器。 */  /* Block live parent register. */
    }
  ...
  if (spdelta < 0) {  /* 不要缩小堆栈帧。 */ /* Don't shrink the stack frame. */
  ...
  if (pass2) {
	...
	if (!ra_hasspill(regsp_spill(rs)))
	  ra_sethint(ir->r, rs);  /* 提示可能已经消失，再次设置它。 */ /* Hint may be gone, set it again. */
	else if (sps_scale(regsp_spill(rs))+spdelta == sps_scale(ir->s))
	  continue; /* 相同的溢出槽，什么也不做。 */ /* Same spill slot, do nothing. */
	...
	ra_save(as, ir, r);
	rset_clear(allow, r);
	if (r == rs) { /* 立即合并匹配的寄存器。 */  /* Coalesce matching registers right now. */
	  ra_free(as, r);
	  rset_clear(live, r);
	} else if (ra_hasspill(regsp_spill(rs))) {
	  pass3 = 1;
  ...
  if (as->topslot > as->T->topslot) { /* 需要检查更高的插槽吗？ */  /* Need to check for higher slot? */
...
{
  SnapNo snapno = as->T->nsnap-1; /* 最后一个快照。 */  /* Last snapshot. */
  SnapShot *snap = &as->T->snap[snapno];/* 最后一个快照的引用。 */
  int gotframe = 0;/* 标记是否有新的栈帧。 */
  BCReg baseslot = asm_baseslot(as, snap, &gotframe); /* 计算基本槽位。 */

  as->topslot = snap->topslot; /* 更新顶部槽位。 */
  checkmclim(as);
  ra_allocref(as, REF_BASE, RID2RSET(RID_BASE)); /* 分配基址寄存器。 */

  if (as->T->link == 0) {
    /* Setup fixed registers for exit to interpreter. */
     /* 设置退出到解释器时的固定寄存器。 */
    const BCIns *pc = snap_pc(&as->T->snapmap[snap->mapofs + snap->nent]); /* 获取最后一条指令。 */
    int32_t mres;
    if (bc_op(*pc) == BC_JLOOP) {  /* 注意：找到更好的方法来处理此情况。 */ /* NYI: find a better way to do this. */
    ...
    ra_allockreg(as, mres, RID_RET);  /* 返回 MULTRES 或 0。 */ /* Return MULTRES or 0. */
  } else if (baseslot) {
    /* Save modified BASE for linking to trace with higher start frame. */
    /* 保存修改后的 BASE 以链接到具有较高起始帧的跟踪。 */
    emit_setgl(as, RID_BASE, jit_base);
  }
  emit_addptr(as, RID_BASE, 8*(int32_t)baseslot);  /* 基地址偏移。 */

  if (as->J->ktrace) { /* 使用最终的 GCtrace 指针修补 ktrace 插槽。 */ /* Patch ktrace slot with the final GCtrace pointer. */
...
void lj_asm_trace(jit_State *J, GCtrace *T)
{
  ...
  /* 移除由于LJ_TRERR_MCODELM而导致的ASM重新启动时剩余的nop和rename。*/
  {
    IRRef nins = T->nins;
    IRIns *ir = &T->ir[nins-1];
    // 如果最后一个IR指令是NOP或RENAME，则向前遍历，直到找到不是这两者的指令
    if (ir->o == IR_NOP || ir->o == IR_RENAME) {
      do { ir--; nins--; } while (ir->o == IR_NOP || ir->o == IR_RENAME);
      T->nins = nins;// 更新跟踪的指令数。
    }
  }

  /* Ensure an initialized instruction beyond the last one for HIOP checks. */
  /* This also allows one RENAME to be added without reallocating curfinal. */
  /* 确保在HIOP检查时有一个初始化的指令。这也允许添加一个RENAME而不重新分配curfinal。*/
  ...
  /* 设置初始状态。复制一些字段以减少间接引用。*/
  as->J = J;
  as->T = T;
  J->curfinal = lj_trace_alloc(J->L, T); // 为新跟踪分配内存。这也会复制IR。 /* This copies the IR, too. */
  as->flags = J->flags;
...
#ifdef LUAJIT_RANDOM_RA
  (void)lj_prng_u64(&J2G(J)->prng);  // 确保在跟踪之间有PRNG步骤。 /* Ensure PRNG step between traces. */
#endif
  ...
  */
   /*
  ** 这是一个循环，因为MCode可能需要被重新汇编多次:
  ** 1. as->realign被设置（并且汇编被中止），如果架构特定的后端希望以不同的方式对齐MCode。
  ** 2. IR是不可移动的，因为MCode嵌入了对IR中各种常量的指针。但在汇编期间可能需要向IR添加RENAME，
  **    这可能会增长并重新分配IR。我们在结束时检查IR（在J->cur.ir中）是否实际增长，调整副本
  **    （在J->curfinal.ir中）的大小并重试。
  */
  for (;;) {
    as->mcp = as->mctop;
#ifdef LUA_USE_ASSERT
    as->mcp_prev = as->mcp;
#endif
    as->ir = J->curfinal->ir; // 使用复制的IR。 /* Use the copied IR. */
    as->curins = J->cur.nins = as->orignins;
#ifdef LUAJIT_RANDOM_RA
    as->prngstate = J2G(J)->prng;  // 必须从相同的状态重新开始。/* Must (re)start from identical state. */
    as->prngbits = 0;
#endif
    ...
    for (as->curins--; as->curins > as->stopins; as->curins--) {
      ...
      if (!ra_used(ir) && !ir_sideeff(ir) && (as->flags & JIT_F_OPT_DCE))
	      continue; // 死代码消除很容易。 /* Dead-code elimination can be soooo easy. */
      if (irt_isguard(ir->t))
	      asm_snap_prep(as);
      RA_DBG_REF();
      checkmclim(as);
    ...
    if (as->realign && J->curfinal->nins >= T->nins)
      continue; // 重试以确保只有MCode需要重新对齐。 /* Retry in case only the MCode needs to be realigned. */
    ...
    if (as->gcsteps > 0) {
      ...
      asm_snap_prep(as); // GC检查是一个守卫。 /* The GC check is a guard. */
      asm_gc_check(as);
    ...
    if (J->curfinal->nins >= T->nins) { // IR没有增长？ /* IR didn't grow? */
      lj_assertA(J->curfinal->nk == T->nk, "unexpected IR constant growth");
      memcpy(J->curfinal->ir + as->orignins, T->ir + as->orignins,
	     (T->nins - as->orignins) * sizeof(IRIns)); // 复制RENAMEs /* Copy RENAMEs. */
    ...
    J->curfinal = NULL; // 以防lj_trace_alloc() OOM。 /* In case lj_trace_alloc() OOMs. */
  ...
  if (as->freeset != RSET_ALL)
    lj_trace_err(as->J, LJ_TRERR_BADRA); // 糟糕！不应该发生。 /* Ouch! Should never happen. */

  /* Set trace entry point before fixing up tail to allow link to self. */
  /* 设置跟踪入口点以修复尾部以允许链接到自身。*/
  T->mcode = as->mcp;
  T->mcloop = as->mcloop ? (MSize)((char *)as->mcloop - (char *)as->mcp) : 0;
  if (as->loopref)
    asm_loop_tail_fixup(as);
  else
    asm_tail_fixup(as, T->link); // 注意：这可能会改变as->mctop！ /* Note: this may change as->mctop! */
