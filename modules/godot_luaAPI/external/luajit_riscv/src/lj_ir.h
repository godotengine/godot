...
** |  op1  |  op2  | t | o | r | s |
** +-------+-------+---+---+---+---+
** |  op12/i/gco32 |   ot  | prev  | (在联合体中的替代字段)
** +-------+-------+---+---+---+---+
** |  TValue/gco64                 | (64位常量的第2个IR槽)
** +---------------+-------+-------+
**        32           16      16
**
** prev 仅在寄存器分配之前有效，然后被重用为 r + s。
*/
...
typedef union IRIns {
  struct {
    ...
    IROpT ot;     /* IR 操作码和类型（与 t 和 o 重叠）。 */
    IRRef1 prev;  /* 同一链中的上一个指令（与 r 和 s 重叠）。 */
  };
  struct {
    IRRef2 op12;  /* IR 操作数 1 和 2（与 op1 和 op2 重叠）。 */
    LJ_ENDIAN_LOHI(
      IRType1 t;  /* IR 类型。 */
    , IROp1 o;    /* IR 操作码。 */
    )
    LJ_ENDIAN_LOHI(
      uint8_t r;  /* 寄存器分配（与 prev 重叠）。 */
    , uint8_t s;  /* 溢出槽分配（与 prev 重叠）。 */
    )
  };
  int32_t i;      /* 32位有符号整数字面值（与 op12 重叠）。 */
  GCRef gcr;      /* GCobj 常量（与 op12 或整个槽 重叠）。 */
  MRef ptr;       /* 指针常量（与 op12 或整个槽 重叠）。 */
  TValue tv;      /* TValue 常量（与整个槽 重叠）。 */
} IRIns;
