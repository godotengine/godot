...
/* 原子部分的 GC 循环，从标记阶段过渡到清理阶段。*/
static void atomic(global_State *g, lua_State *L)
{
  size_t udsize;

  gc_mark_uv(g);  /* 需要重新标记打开的上值（线程可能已经死亡）。 */
  gc_propagate_gray(g);  /* 传播任何遗留的对象。 */

  setgcrefr(g->gc.gray, g->gc.weak);  /* 清空弱表列表。 */
  setgcrefnull(g->gc.weak);
  lj_assertG(!iswhite(obj2gco(mainthread(g))), "main thread turned white");
  gc_markobj(g, L);  /* 标记运行中的线程。 */
  gc_traverse_curtrace(g);  /* 遍历当前追踪。 */
  gc_mark_gcroot(g);  /* 标记 GC 根（再次）。 */
  gc_propagate_gray(g);  /* 传播上述所有标记。 */

  setgcrefr(g->gc.gray, g->gc.grayagain);  /* 清空第二次机会列表。 */
  setgcrefnull(g->gc.grayagain);
  gc_propagate_gray(g);  /* 传播它。 */

  udsize = lj_gc_separateudata(g, 0);  /* 将需要被 finalizer 处理的 userdata 分离出来。 */
  gc_mark_mmudata(g);  /* 标记它们。 */
  udsize += gc_propagate_gray(g);  /* 然后传播标记。 */

  /* 所有标记完成，清除弱表。 */
  gc_clearweak(g, gcref(g->gc.weak));

  lj_buf_shrink(L, &g->tmpbuf);  /* 缩小临时缓冲区。 */

  /* 准备清理阶段。 */
  g->gc.currentwhite = (uint8_t)otherwhite(g);  /* 翻转当前白色。 */
  g->strempty.marked = g->gc.currentwhite;
  setmref(g->gc.sweep, &g->gc.root);
  g->gc.estimate = g->gc.total - (GCSize)udsize;  /* 初始估算值。 */
}

/* GC 状态机。为每一步执行返回一个成本估算。*/
static size_t gc_onestep(lua_State *L)
{
  ...
  switch (g->gc.state) {
  case GCSpause:
    gc_mark_start(g);  /* 开始新的 GC 循环，标记所有 GC 根。 */
    return 0;
  case GCSpropagate:
    if (gcref(g->gc.gray) != NULL)
      return propagatemark(g);  /* 传播一个灰色对象。 */
    g->gc.state = GCSatomic;  /* 标记阶段结束。 */
    return 0;
  case GCSatomic:
    if (tvref(g->jit_base))  /* 在追踪期间不运行原子阶段。 */
      return LJ_MAX_MEM;
    atomic(g, L);
    g->gc.state = GCSsweepstring;  /* 开始清理阶段。 */
    g->gc.sweepstr = 0;
    return 0;
  case GCSsweepstring: {
    ...
    if (g->gc.sweepstr > g->str.mask)
      g->gc.state = GCSsweep;  /* 所有字符串哈希链已清理。 */
  ...
  case GCSsweep: {
    ...
    // 如果没有待清理的对象，执行一些附加操作，然后结束 GC 循环。
    if (gcref(*mref(g->gc.sweep, GCRef)) == NULL) {
      if (g->str.num <= (g->str.mask >> 2) && g->str.mask > LJ_MIN_STRTAB * 2 - 1)
        lj_str_resize(L, g->str.mask >> 1);  /* 缩小字符串表。 */
      if (gcref(g->gc.mmudata)) {  /* 需要执行任何 finalization 吗？ */
        g->gc.state = GCSfinalize;
#if LJ_HASFFI
        g->gc.nocdatafin = 1;
#endif
      } else {  /* 否则跳过该阶段以帮助 JIT。 */
        g->gc.state = GCSpause;  /* 结束 GC 循环。 */
        g->gc.debt = 0;
      }
    }
  ...
  case GCSfinalize:
    if (gcref(g->gc.mmudata) != NULL) {
      ...
      // 遍历并调用一个 userdata 对象的 finalizer。
      if (tvref(g->jit_base))  /* 在追踪期间不调用 finalizer。 */
        return LJ_MAX_MEM;
      gc_finalize(L);
      ...
      // 减少 GC 估算值，表示执行了一个 finalizer。
      if (g->gc.estimate > GCFINALIZECOST)
        g->gc.estimate -= GCFINALIZECOST;
      
      // 返回执行 finalizer 的估算成本。
      return GCFINALIZECOST;
    }
    
#if LJ_HASFFI
    // 调整 C 数据的 finalizer 表。
