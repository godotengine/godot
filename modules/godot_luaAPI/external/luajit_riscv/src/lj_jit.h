...
typedef struct jit_State {
  GCtrace cur;          /* 当前跟踪。*/
  GCtrace *curfinal;    /* 当前跟踪的最终地址（在汇编期间设置）。*/

  lua_State *L;         /* 当前 Lua 状态。*/
  const BCIns *pc;      /* 当前 PC。*/
  GCfunc *fn;           /* 当前函数。*/
  GCproto *pt;          /* 当前原型。*/
  TRef *base;           /* 当前帧基址，指向 J->slots。*/

  uint32_t flags;       /* JIT 引擎标志。*/
  BCReg maxslot;        /* 相对于 baseslot。*/
  BCReg baseslot;       /* 当前帧基址，偏移量为 J->slots。*/

  uint8_t mergesnap;    /* 允许与下一个快照合并。*/
  uint8_t needsnap;     /* 在记录下一条字节码之前需要快照。*/
  IRType1 guardemit;    /* 已发出指令的 IRT_GUARD 的累积。*/
  uint8_t bcskip;       /* 跳过的字节码指令数。*/

  FoldState fold;       /* 折叠状态。*/

  const BCIns *bc_min;  /* 根跟踪的允许字节码范围的起始位置。*/
  MSize bc_extent;      /* 范围的扩展。*/

  TraceState state;      /* 跟踪编译器状态。*/

  int32_t instunroll;    /* 不稳定循环的展开计数器。*/
  int32_t loopunroll;    /* 在辅助跟踪中展开循环操作的计数器。*/
  int32_t tailcalled;    /* 连续的尾调用次数。*/
  int32_t framedepth;    /* 当前帧深度。*/
  int32_t retdepth;      /* 返回帧深度（RETF 的计数）。*/

#if LJ_K32__USED
  uint32_t k32[LJ_K32__MAX];  /* 后端使用的常见 4 字节常数。*/
#endif
  TValue ksimd[LJ_KSIMD__MAX*2+1];  /* 16 字节对齐的 SIMD 常数。*/
#if LJ_K64__USED
  TValue k64[LJ_K64__MAX];  /* 常见的 8 字节常数。*/
#endif

  IRIns *irbuf;        /* 临时 IR 指令缓冲区。偏移量为 REF_BIAS。*/
  IRRef irtoplim;      /* 指令缓冲区的上限（带有偏移量）。*/
  IRRef irbotlim;      /* 指令缓冲区的下限（带有偏移量）。*/
  IRRef loopref;       /* 最后的循环引用或最终 LOOP 的引用（或为 0）。*/

  MSize sizesnap;      /* 临时快照缓冲区的大小。*/
  SnapShot *snapbuf;   /* 临时快照缓冲区。*/
  SnapEntry *snapmapbuf;  /* 临时快照映射缓冲区。*/
  MSize sizesnapmap;   /* 临时快照映射缓冲区的大小。*/

  PostProc postproc;   /* 执行后需要的后处理。*/
#if LJ_SOFTFP32 || (LJ_32 && LJ_HASFFI)
  uint8_t needsplit;   /* 需要 SPLIT pass。*/
#endif
  uint8_t retryrec;    /* 重试记录。*/

  GCRef *trace;        /* 跟踪数组。*/
  TraceNo freetrace;    /* 下一个空闲跟踪的扫描开始。*/
  MSize sizetrace;      /* 跟踪数组的大小。*/
  IRRef1 ktrace;        /* 具有 GCtrace 的 KGC 的引用。*/

  IRRef1 chain[IR__MAX];  /* IR 指令跳过列表链锚点。*/
  TRef slot[LJ_MAX_JSLOTS+LJ_STACK_EXTRA];  /* 栈槽映射。*/

  int32_t param[JIT_P__MAX];  /* JIT 引擎参数。*/

  MCode *exitstubgroup[LJ_MAX_EXITSTUBGR];  /* 退出存根组地址。*/
  HotPenalty penalty[PENALTY_SLOTS];  /* 惩罚槽。*/
  uint32_t penaltyslot;  /* 惩罚槽的循环索引。*/

#ifdef LUAJIT_ENABLE_TABLE_BUMP
  RBCHashEntry rbchash[RBCHASH_SLOTS];  /* 反向字节码映射。*/
#endif

  BPropEntry bpropcache[BPROP_SLOTS];  /* 反向传播缓存槽。*/
  uint32_t bpropslot;  /* 反向传播缓存槽的循环索引。*/

  ScEvEntry scev;      /* 标量演化分析缓存槽。*/

  const BCIns *startpc;  /* 起始指令的字节码 PC。*/
  TraceNo parent;      /* 当前辅助跟踪的父级（根跟踪为 0）。*/
  ExitNo exitno;        /* 当前辅助跟踪的父级的退出编号。*/
  int exitcode;         /* 从展开的跟踪中解旋的退出代码。*/

  BCIns *patchpc;       /* 待处理重新打补丁的 PC。*/
  BCIns patchins;       /* 待处理重新打补丁的指令。*/

  int mcprot;           /* 当前 mcode 区域的保护。*/
  MCode *mcarea;        /* 当前 mcode 区域的基地址。*/
  MCode *mctop;         /* 当前 mcode 区域的顶部。*/
  MCode *mcbot;         /* 当前 mcode 区域的底部。*/
  size_t szmcarea;      /* 当前 mcode 区域的大小。*/
  size_t szallmcarea;   /* 所有分配的 mcode 区域的总大小。*/

  TValue errinfo;       /* 跟踪错误的附加信息元素。*/

#if LJ_HASPROFILE
  GCproto *prev_pt;     /* 先前的原型。*/
  BCLine prev_line;     /* 先前的行号。*/
  int prof_mode;        /* 探查模式：0、'f'、'l'。*/
#endif
} jit_State;
