...
thread_local const HashSet<RID> *JoltPhysicsServer3D::exclude_bodies = nullptr;
thread_local const HashSet<ObjectID> *JoltPhysicsServer3D::exclude_objects = nullptr;

bool JoltPhysicsServer3D::body_test_motion_is_excluding_body(RID p_body) const {
	return exclude_bodies && exclude_bodies->has(p_body);
}

bool JoltPhysicsServer3D::body_test_motion_is_excluding_object(ObjectID p_object) const {
...
void JoltPhysicsServer3D::body_get_collision_exceptions(RID p_body,List<RID> *p_exceptions) {
	...
	auto exceptions = body->get_collision_exceptions();
...
bool JoltPhysicsServer3D::body_test_motion(	RID p_body,const MotionParameters &p_parameters, PhysicsServer3DExtensionMotionResult* p_result
	)   {
	JoltBodyImpl3D* body = body_owner.get_or_null(p_body);
	ERR_FAIL_NULL_D(body);
	...
	bool ret = space->get_direct_state()->test_body_motion(
		*body,
		p_parameters.from,
		p_parameters.motion,
		(float)p_parameters.margin,
...
bool JoltPhysicsServer3D::soft_body_is_point_pinned(RID p_body, int32_t p_point_index)
	const {
	JoltSoftBodyImpl3D* body = soft_body_owner.get_or_null(p_body);
	ERR_FAIL_NULL_D(body);
