...
public:
	...
	JPH_INLINE					Mat44(const Vec4Arg& inC1, const Vec4Arg& inC2, const Vec4Arg& inC3, const Vec4Arg& inC4);
	JPH_INLINE					Mat44(const Vec4Arg& inC1, const Vec4Arg& inC2, const Vec4Arg& inC3, const Vec3Arg& inC4);
								Mat44(const Mat44 &inM2) = default;
	Mat44 &						operator = (const Mat44 &inM2) = default;
	JPH_INLINE					Mat44(const Type& inC1, const Type& inC2, const Type& inC3, const Type& inC4);
	...
	static JPH_INLINE Mat44		sRotation(const Vec3Arg& inAxis, float inAngle);

	/// Rotate from quaternion
	static JPH_INLINE Mat44		sRotation(const QuatArg& inQuat);

	/// Get matrix that translates
	static JPH_INLINE Mat44		sTranslation(const Vec3Arg& inV);

	/// Get matrix that rotates and translates
	static JPH_INLINE Mat44		sRotationTranslation(const QuatArg& inR, const Vec3Arg& inT);

	/// Get inverse matrix of sRotationTranslation
	static JPH_INLINE Mat44		sInverseRotationTranslation(const QuatArg& inR, const Vec3Arg& inT);
	...
	static JPH_INLINE Mat44		sScale(const Vec3Arg& inV);

	/// Get outer product of inV and inV2 (equivalent to \f$inV1 \otimes inV2\f$)
	static JPH_INLINE Mat44		sOuterProduct(const Vec3Arg& inV1, const Vec3Arg& inV2);

	/// Get matrix that represents a cross product \f$A \times B = \text{sCrossProduct}(A) \: B\f$
	static JPH_INLINE Mat44		sCrossProduct(const Vec3Arg& inV);

	/// Returns matrix ML so that \f$ML(q) \: p = q \: p\f$ (where p and q are quaternions)
	static JPH_INLINE Mat44		sQuatLeftMultiply(const QuatArg& inQ);

	/// Returns matrix MR so that \f$MR(q) \: p = p \: q\f$ (where p and q are quaternions)
	static JPH_INLINE Mat44		sQuatRightMultiply(const QuatArg& inQ);
	...
	static JPH_INLINE Mat44		sLookAt(const Vec3Arg& inPos, const Vec3Arg& inTarget, const Vec3Arg& inUp);
	...
	/// Comparison
	JPH_INLINE bool				operator == (const Mat44Arg& inM2) const;
	JPH_INLINE bool				operator != (const Mat44Arg& inM2) const						{ return !(*this == inM2); }

	/// Test if two matrices are close
	JPH_INLINE bool				IsClose(const Mat44Arg& inM2, float inMaxDistSq = 1.0e-12f) const;

	/// Multiply matrix by matrix
	JPH_INLINE Mat44			operator * (const Mat44Arg& inM) const;

	/// Multiply vector by matrix
	JPH_INLINE Vec3				operator * (const Vec3Arg& inV) const;
	JPH_INLINE Vec4				operator * (const Vec4Arg& inV) const;

	/// Multiply vector by only 3x3 part of the matrix
	JPH_INLINE Vec3				Multiply3x3(const Vec3Arg& inV) const;

	/// Multiply vector by only 3x3 part of the transpose of the matrix (\f$result = this^T \: inV\f$)
	JPH_INLINE Vec3				Multiply3x3Transposed(const Vec3Arg& inV) const;

	/// Multiply 3x3 matrix by 3x3 matrix
	JPH_INLINE Mat44			Multiply3x3(const Mat44Arg& inM) const;

	/// Multiply transpose of 3x3 matrix by 3x3 matrix (\f$result = this^T \: inM\f$)
	JPH_INLINE Mat44			Multiply3x3LeftTransposed(const Mat44Arg& inM) const;

	/// Multiply 3x3 matrix by the transpose of a 3x3 matrix (\f$result = this \: inM^T\f$)
	JPH_INLINE Mat44			Multiply3x3RightTransposed(const Mat44Arg& inM) const;

	/// Multiply matrix with float
	JPH_INLINE Mat44			operator * (float inV) const;
	friend JPH_INLINE Mat44		operator * (float inV, const Mat44Arg& inM)					{ return inM * inV; }
	...
	/// Per element addition of matrix
	JPH_INLINE Mat44 &			operator += (const Mat44Arg& inM);
	...
	JPH_INLINE void				SetTranslation(const Vec3Arg& inV)								{ mCol[3] = Vec4(inV, 1.0f); }
	JPH_INLINE Vec3				GetDiagonal3() const									{ return Vec3(mCol[0][0], mCol[1][1], mCol[2][2]); }
	JPH_INLINE void				SetDiagonal3(const Vec3Arg& inV)								{ mCol[0][0] = inV.GetX(); mCol[1][1] = inV.GetY(); mCol[2][2] = inV.GetZ(); }
	JPH_INLINE Vec4				GetDiagonal4() const									{ return Vec4(mCol[0][0], mCol[1][1], mCol[2][2], mCol[3][3]); }
	JPH_INLINE void				SetDiagonal4(const Vec4Arg& inV)								{ mCol[0][0] = inV.GetX(); mCol[1][1] = inV.GetY(); mCol[2][2] = inV.GetZ(); mCol[3][3] = inV.GetW(); }
	JPH_INLINE Vec3				GetColumn3(uint inCol) const							{ JPH_ASSERT(inCol < 4); return Vec3(mCol[inCol]); }
	JPH_INLINE void				SetColumn3(uint inCol, const Vec3Arg& inV)						{ JPH_ASSERT(inCol < 4); mCol[inCol] = Vec4(inV, inCol == 3? 1.0f : 0.0f); }
	JPH_INLINE Vec4				GetColumn4(uint inCol) const							{ JPH_ASSERT(inCol < 4); return mCol[inCol]; }
	JPH_INLINE void				SetColumn4(uint inCol, const Vec4Arg& inV)						{ JPH_ASSERT(inCol < 4); mCol[inCol] = inV; }
	...
	JPH_INLINE bool				SetInversed3x3(const Mat44Arg& inM);
	...
	JPH_INLINE Mat44			PreTranslated(const Vec3Arg& inTranslation) const;
	...
	JPH_INLINE Mat44			PostTranslated(const Vec3Arg& inTranslation) const;

	/// Scale a matrix: result = this * Mat44::sScale(inScale)
	JPH_INLINE Mat44			PreScaled(const Vec3Arg& inScale) const;

	/// Scale a matrix: result = Mat44::sScale(inScale) * this
	JPH_INLINE Mat44			PostScaled(const Vec3Arg& inScale) const;
