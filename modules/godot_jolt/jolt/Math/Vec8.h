...
public:
	...
	JPH_INLINE					Vec8(const Vec4Arg& inLo, const Vec4Arg& inHi);
	...
	static JPH_INLINE Vec8		sSplatX(const Vec4Arg& inV);

	/// Replicate the Y component of inV to all components
	static JPH_INLINE Vec8		sSplatY(const Vec4Arg& inV);

	/// Replicate the Z component of inV to all components
	static JPH_INLINE Vec8		sSplatZ(const Vec4Arg& inV);

	/// Calculates inMul1 * inMul2 + inAdd
	static JPH_INLINE Vec8		sFusedMultiplyAdd(const Vec8Arg& inMul1, const Vec8Arg& inMul2, const Vec8Arg& inAdd);

	/// Component wise select, returns inV1 when highest bit of inControl = 0 and inV2 when highest bit of inControl = 1
	static JPH_INLINE Vec8		sSelect(const Vec8Arg& inV1, const Vec8Arg& inV2, const UVec8Arg& inControl);

	/// Component wise min
	static JPH_INLINE Vec8		sMin(const Vec8Arg& inV1, const Vec8Arg& inV2);

	/// Component wise max
	static JPH_INLINE Vec8		sMax(const Vec8Arg& inV1, const Vec8Arg& inV2);

	/// Less than
	static JPH_INLINE UVec8		sLess(const Vec8Arg& inV1, const Vec8Arg& inV2);

	/// Greater than
	static JPH_INLINE UVec8		sGreater(const Vec8Arg& inV1, const Vec8Arg& inV2);
	...
	JPH_INLINE Vec8				operator * (const Vec8Arg& inV2) const;
