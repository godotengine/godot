...
// Object arguments are resolved to their raw pointer via extract_ptr (from the ptr cache),
// since .NET tracks objects by ptr rather than ObjectID to avoid 64-bit overflow issues.
void CrossRuntimeEventSignal::_on_any_signal(const Variant **p_args, int p_argcount, Callable::CallError &r_error) {
    r_error.error = Callable::CallError::CALL_OK;
    if (p_argcount < 2) return;
