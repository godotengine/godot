...
const char* callable_get_method(uintptr_t ptr) {
    Callable* c = reinterpret_cast<Callable*>(ptr);
    if (!c || !c->is_valid()) return nullptr;
    String method = c->get_method();
    if (method.is_empty()) return nullptr;
...
void variant_as_basis(const Variant* v, float* out9) {
    ...
    out9[0] = b.rows[0].x; out9[1] = b.rows[0].y; out9[2] = b.rows[0].z;
    out9[3] = b.rows[1].x; out9[4] = b.rows[1].y; out9[5] = b.rows[1].z;
    out9[6] = b.rows[2].x; out9[7] = b.rows[2].y; out9[8] = b.rows[2].z;
}
...
void variant_as_transform3d(const Variant* v, float* out12) {
    ...
    out12[0]  = t.basis.rows[0].x; out12[1]  = t.basis.rows[0].y; out12[2]  = t.basis.rows[0].z;
    out12[3]  = t.basis.rows[1].x; out12[4]  = t.basis.rows[1].y; out12[5]  = t.basis.rows[1].z;
    out12[6]  = t.basis.rows[2].x; out12[7]  = t.basis.rows[2].y; out12[8]  = t.basis.rows[2].z;
    out12[9]  = t.origin.x;        out12[10] = t.origin.y;        out12[11] = t.origin.z;
}
