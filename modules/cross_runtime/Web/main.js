(async function () {
  ...
  // Start the Godot engine – no click overlay, just go.
  ...
  // Load the bridge helper modules in parallel
  const [
    { StringMarshaller },
  ...
  globalThis.__callGodot = function (fn, ...rawArgs) {
    ...
    if (!wasmFn) throw new Error(`Missing WASM export: ${fn}`);

    let args = rawArgs;
    if (rawArgs.length === 1 && Array.isArray(rawArgs[0])) args = rawArgs[0];
    ...
    try {
      ...
      for (const vptr of variantPtrs) TheGodotModule._variant_destroy(vptr);
      for (const ptr of tempPtrs) TheGodotModule._free(ptr);
    }
  };
  ...
  if (!exports?.Interop?.InitInterop) throw new Error("InitInterop missing");

  exports.Interop.RunTypeTests();
