 ...
 */
export function decodeReturn(M, raw, returnKind, readUTF8, decodeVariant) {
    switch (returnKind) {
        case "void":     return null;
        case "i64": {
            console.log("[decodeReturn i64] raw =", raw, "| type =", typeof raw);
            const result = typeof raw === "bigint" ? Number(raw) : raw;
            console.log("[decodeReturn i64] result =", result, "| type =", typeof result);
            return result;
        }
    ...
    case "Callable": {
        ...
        const targetId = M._callable_get_target_id(ptr);
        const methodPtr = M._callable_get_method(ptr);
