 ...
 */
function bulkCopy(M, dotnetModule, a, tempPtrs) {
    const byteLength = a._length;
    const ptr = M._malloc(byteLength);
    M.HEAPU8.set(dotnetModule.HEAPU8.subarray(a._pointer, a._pointer + byteLength), ptr);
    tempPtrs.push(ptr);
 ...
 * @param {object}   M             – Emscripten Module
 ...
 * @param {string}   kind          – marshalling kind
 ...
 */
export function marshalArg(M, sm, tempPtrs, variantPtrs, marshaledArgs, kind, a, i, marshalVarargs, nextArg) {
    switch (kind) {
case "varargs": {
    const [ptr, count] = marshalVarargs(M, a, tempPtrs, variantPtrs);
    ...
    break;
        ...
        case "cstring":
            ...
            break;
        case "ptr":
            if (a == null)                  marshaledArgs.push(0);
            else if (typeof a === "bigint") marshaledArgs.push(Number(a));
            else                            marshaledArgs.push(Math.trunc(Number(a)));
            break;
        case "i64":
            if (a == null)                  marshaledArgs.push(0n);
            else if (typeof a === "bigint") marshaledArgs.push(a);
            else                            marshaledArgs.push(BigInt(Math.trunc(Number(a))));
            break;
        case "i32": marshaledArgs.push(Math.trunc(Number(a ?? 0))); break;
        case "f32": marshaledArgs.push(Math.fround(Number(a ?? 0))); break;
        case "f64": marshaledArgs.push(Number(a ?? 0)); break;
        case "bool": marshaledArgs.push(a ? 1 : 0); break;
        ...
        case "PackedByteArray": {
            ...
            const { ptr, byteLength } = bulkCopy(M, dotnetModule, a, tempPtrs);
        ...
        case "PackedStringArray": {
            ...
            const ptrTable = M._malloc(count * 4); // count * sizeof(char*)
            for (let j = 0; j < count; j++) {
                const cstr = sm.marshal(strings[j] == null ? "" : String(strings[j]));
                M.HEAPU32[(ptrTable >> 2) + j] = cstr;
            }
        ...
        case "Array": {
            ...
            const ptrsPtr = M._malloc(n * 4);
            const ptrsView = new Uint32Array(M.HEAPU8.buffer, ptrsPtr, n);
            for (let j = 0; j < n; j++) {
                const vptr = constructVariant(M, values[j]);
                ptrsView[j] = vptr;
        ...
        case "Dictionary": {
            ...
            const n = Math.trunc(Number(count));
            const ptrTable = M._malloc(n * 4);
            for (let j = 0; j < n; j++)
                M.HEAPU32[(ptrTable >> 2) + j] = sm.marshal(String(keys[j] ?? ""));
            tempPtrs.push(ptrTable);
            const ptrsPtr = M._malloc(n * 4);
            const ptrsView = new Uint32Array(M.HEAPU8.buffer, ptrsPtr, n);
            for (let j = 0; j < n; j++) {
        ...
        default:
            ...
            else if (typeof a === "number")  marshaledArgs.push(a);
            else if (typeof a === "boolean") marshaledArgs.push(a ? 1 : 0);
            else if (typeof a === "string")  marshaledArgs.push(sm.marshal(a));
            else throw new TypeError(`[marshal] arg[${i}] unknown kind "${kind}"`);
    }
}