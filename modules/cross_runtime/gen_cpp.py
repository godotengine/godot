...
FLATTENED_STRUCTS = {
    ...
    "AABB":         (("position_x", "float"), ("position_y", "float"), ("position_z", "float"),
                     ("size_x", "float"),     ("size_y", "float"),     ("size_z", "float")),
    "Transform2D":  (("x_x", "float"), ("x_y", "float"),
                     ("y_x", "float"), ("y_y", "float"),
                     ("origin_x", "float"), ("origin_y", "float")),
    "Transform3D":  (("bx_x", "float"), ("bx_y", "float"), ("bx_z", "float"),
                     ("by_x", "float"), ("by_y", "float"), ("by_z", "float"),
                     ("bz_x", "float"), ("bz_y", "float"), ("bz_z", "float"),
                     ("origin_x", "float"), ("origin_y", "float"), ("origin_z", "float")),
    "Quaternion":   (("x", "float"), ("y", "float"), ("z", "float"), ("w", "float")),
    "Basis":        (("x_x", "float"), ("x_y", "float"), ("x_z", "float"),
                     ("y_x", "float"), ("y_y", "float"), ("y_z", "float"),
                     ("z_x", "float"), ("z_y", "float"), ("z_z", "float")),
}
...
FLATTENED_STRUCT_TYPE = {
    ...
    "Color": "Color", "Plane": "Plane", "AABB": "AABB",
...
def packed_array_reconstruction(godot_type: str, safe_name: str,
                                data_ptr: str, size_var: str, lines: list) -> None:
    ...
    if elem_size is not None:
    ...
    elif godot_type in ("PackedVector2Array", "PackedVector3Array",
        ...
        if godot_type == "PackedVector2Array":
            ...
            lines.append(f"    {safe_name}.resize({size_var});")
            lines.append(f"    for (int i = 0; i < {size_var}; i++) {{")
            lines.append(f"        {safe_name}.set(i, Color({data_ptr}[i*4], {data_ptr}[i*4+1], {data_ptr}[i*4+2], {data_ptr}[i*4+3]));")
            lines.append(f"    }}")
        else:  # PackedVector4Array
            lines.append(f"    PackedVector4Array {safe_name};")
            lines.append(f"    {safe_name}.resize({size_var});")
            lines.append(f"    for (int i = 0; i < {size_var}; i++) {{")
            lines.append(f"        {safe_name}.set(i, Vector4({data_ptr}[i*4], {data_ptr}[i*4+1], {data_ptr}[i*4+2], {data_ptr}[i*4+3]));")
            lines.append(f"    }}")
    else:
...
CXX_KEYWORDS = {
    "class", "new", "delete", "default", "operator", "template", "typename",
    "enum", "union", "struct", "private", "protected", "public", "friend",
    "virtual", "explicit", "constexpr", "static", "const", "volatile", "export",
    "char", "int", "float", "double", "long", "short", "signed", "unsigned",
    "bool", "void", "auto", "register", "extern", "mutable", "thread_local",
    "switch", "case", "break", "continue", "goto", "return", "if", "else",
    "while", "do", "for", "try", "catch", "throw", "typedef", "using",
    "sizeof", "alignas", "alignof", "decltype", "noexcept", "nullptr",
    "concept", "requires", "co_await", "co_return", "co_yield", "consteval",
    "constinit", "and", "or", "xor", "not", "bitand", "bitor", "compl",
    "args", "type",
}
...
def early_return(c_ret: str) -> str:
    ...
    if c_ret == "void":         return "return;"
    if c_ret == "Variant*":    return "return nullptr;"
    if c_ret == "bool":         return "return false;"
    if c_ret in ("int64_t", "double"): return "return 0;"
    if c_ret == "const char*": return 'return "";'
...
def is_object(godot_type: str) -> bool:
    ...
    if (godot_type in PARAM_TYPE_MAP or
        godot_type in VALUE_TYPE_RETURN or
        godot_type in STRING_RETURN or
        godot_type in FLATTENED_STRUCTS or
        godot_type in VARIANT_RETURN_TYPES):
        return False
...
def emit_return_stmt(lines: list, return_type: str, is_refcounted_ret: bool) -> None:
    """
...
def write_class_file(class_name: str, methods: list, output_dir: Path,
                     is_resource_loader: bool = False,
                     is_refcounted_class: bool = False) -> None:
    ...
    for method in methods:
        ...
        is_refcounted_ret = (
            method.get("is_refcounted", False) or
            is_refcounted(return_type) or
            (is_object(return_type) and return_type == class_name and is_refcounted_class)
        )
        ...
        # ── Object / bind lookup ────────────────────────────────────────────
        lines.append(
            "    Object *_obj_ = ObjectDB::get_instance("
            "((Object*)(uintptr_t)p_object_ptr)->get_instance_id());"
        )
        lines.append(f"    if (!_obj_) {{ {bail} }}")
        lines.append(
            f'    MethodBind *_bind_ = ClassDB::get_method("{class_name}", "{method_name}");'
        )
...
def generate_all_class_files(json_path: Path, output_dir: Path) -> None:
    ...
    for cls in class_list:
        ...
        write_class_file(class_name, methods, output_dir,
