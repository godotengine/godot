...
public static class Helpers
{
    ...
    // Resets the CMD_OFFSET and STATUS_OFFSET to 0 at the start of the program
    ...
    // Bytes are pushed to or pulled out off the offsets
    ...
    // The Standalone WriteSignal is not available, To write signals, it uses WriteVariant directly
    public static object ReadSignal(int offset)
    {
        int len = ReadInt32(offset);

        return (CustomSignal)VariantHandling.Decode(offset + 4);
    }
    ...
    // compatibility alias
    public static void WriteFloatArray(int offset, float[] values) => WritePackedFloat32Array(offset, values);
