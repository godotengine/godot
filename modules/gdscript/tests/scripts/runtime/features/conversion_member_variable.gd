...
func test_external_property_untyped_assignee(f):
	f.foo = "4"
	Utils.check(typeof(f.foo) == TYPE_STRING_NAME)
	Utils.check(f.foo == &"4")
...
func _make_untyped(value: String, type):
	match type:
		TYPE_STRING:
			return String(value)
		TYPE_STRING_NAME:
...
func test_external_property_untyped_value(f: Foo):
	f.foo = _make_untyped("5", TYPE_STRING)
	Utils.check(typeof(f.foo) == TYPE_STRING_NAME)
	Utils.check(f.foo == &"5")

	f.bar = _make_untyped("5", TYPE_STRING_NAME)
	Utils.check(typeof(f.bar) == TYPE_STRING)
	Utils.check(f.bar == "5")
...
func test():
	...
	test_external_property_untyped_assignee(foo)
	test_external_property_untyped_value(foo)
	test_static(Foo)
