...
- A `.out.gd` file with GDScript code _after_ the code action is performed.
...
Additionally, there should be an `[apply]` section with three keys: `line`, `group_idx`, and `action_idx`, corresponding to integer values. The code actions system reports groups of code actions for each line, and these three identify a single code action to perform. For example, when getting code actions for a script with an `UNUSED_VARIABLE` warning on line 4, the code actions returned for line 4 may look like so:
...
To test the action `Remove variable declaration`, we would go to the line the code action is available for (line 4 here, so we have `line=4`), the first group for that line(`group_idx=0`), and the second action within that group (`action_idx=1`). Thus, the `[apply]` section of the `.cfg` file would look like:
