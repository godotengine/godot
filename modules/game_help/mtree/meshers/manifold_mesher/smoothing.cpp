...
namespace Mtree::MeshProcessing::Smoothing
{
    ...
    void smooth_mesh_once(std::vector<Eigen::Vector3f>* result, const std::vector<Eigen::Vector3f>* previous_iteration, const std::vector<std::vector<int>>& neighbourhoods, float factor, std::vector<float>* weights=nullptr)
    {
    ...
    void smooth_mesh(TreeMesh& mesh, const int iterations, const float factor, std::vector<float>* weights)
    {
        ...
        std::vector<Eigen::Vector3f>* previous_iteration = &mesh.vertices;
        std::vector<Eigen::Vector3f> buffer = mesh.vertices;
        std::vector<Eigen::Vector3f>* result = &buffer;
