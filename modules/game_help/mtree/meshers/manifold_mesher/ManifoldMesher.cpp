...
namespace 
{
    ...
    CircleDesignator add_circle(const Eigen::Vector3f& node_position, const TreeNode& node, float factor, const int radial_n_points, TreeMesh& mesh, const float uv_y)
    {
        const Eigen::Vector3f& right = node.tangent;
        int vertex_index = mesh.vertices.size();
        int uv_index = mesh.uvs.size();
        Eigen::Vector3f up = node.tangent.cross(node.direction);
        Eigen::Vector3f circle_position = node_position + node.length * factor * node.direction;
        ...
        auto& direction_attr = *static_cast<Attribute<Eigen::Vector3f>*> (mesh.attributes[AttributeNames::direction].get());
        for (size_t i = 0; i < radial_n_points; i++)
        {
            float angle = (float)i / radial_n_points * 2 * M_PI;
            Eigen::Vector3f point = cos(angle) * right + sin(angle) * up;
    ...
    bool is_index_in_branch_mask(const std::vector<IndexRange>& mask, const int index, const int radial_n_points)
    {
    ...
    void bridge_circles(const CircleDesignator& first_circle, const CircleDesignator& second_circle, const int radial_n_points, TreeMesh& mesh, std::vector<IndexRange>* mask = nullptr)
    {
    ...
    float get_branch_angle_around_parent(const TreeNode& parent, const TreeNode& branch)
    {
        Eigen::Vector3f projected_branch_dir = Geometry::projected_on_plane(branch.direction, parent.direction).normalized();
        auto& right = parent.tangent;
        Eigen::Vector3f up = right.cross(parent.direction);
    ...
    std::vector<IndexRange> get_children_ranges(const TreeNode& node, const int radial_n_points)
    {
    ...
    std::vector<int> get_child_index_order(const CircleDesignator& parent_base, const int child_radial_n, const IndexRange child_range, const NodeChild& child, const TreeNode& parent, const TreeMesh& mesh)
    {
    ...
    void add_child_base_geometry(const std::vector<int>& child_base_indices, const CircleDesignator& child_base, const float child_radius, const Eigen::Vector3f& child_pos, const int offset, const float smooth_amount, MesTreeMeshh& mesh)
    {
        ...
        auto& direction_attr = *static_cast<Attribute<Eigen::Vector3f>*> (mesh.attributes[ManifoldMesher::AttributeNames::direction].get());

        Eigen::Vector3f direction = (mesh.vertices[child_base_indices[2]] - mesh.vertices[child_base_indices[0]]).cross(mesh.vertices[child_base_indices[1]] - mesh.vertices[child_base_indices[0]]).normalized();
        
        Eigen::Vector3f child_base_center{ 0,0,0 };
        ...
        for (int i = 0; i < child_base.radial_n; i++)
        {
            ...
            Eigen::Vector3f vertex = mesh.vertices[child_base_indices[(size_t)index]];
    ...
    float get_child_twist(const TreeNode& child, const TreeNode& parent)
    {
        Eigen::Vector3f projected_parent_dir = Geometry::projected_on_plane(parent.direction, child.direction).normalized();
        auto& right = projected_parent_dir;
        Eigen::Vector3f up = right.cross(child.direction);
        float cos_angle = child.tangent.dot(right);
        float sin_angle = child.tangent.dot(up);
        return std::fmod(std::atan2(sin_angle, cos_angle) + 2 * M_PI, 2 * M_PI);
    }

    int add_child_base_uvs(float parent_uv_y, const TreeNode& parent, const NodeChild& child, const IndexRange child_range, const int child_radial_n, const int parent_radial_n, TreeMesh& mesh)
    {
        ...
        Eigen::Vector2f uv_circle_center{ (child_range.min_index + (child_radial_n / 4.f - .5f))/parent_radial_n, parent_uv_y + uv_growth / 2 };
        ...
        for (size_t i = 0; i < child_radial_n; i++) // inner uvs
        {
            ...
            Eigen::Vector2f uv_position = Eigen::Vector2f{ cos(angle), sin(angle) } *uv_circle_radius + uv_circle_center;
    ...
    CircleDesignator add_child_circle(const TreeNode& parent, const NodeChild& child, const Eigen::Vector3f& child_pos, const Eigen::Vector3f& parent_pos, const CircleDesignator& parent_base, const IndexRange child_range, const float uv_y, TreeMesh& mesh)
    {
    ...
    Eigen::Vector3f get_side_child_position(const TreeNode& parent, const NodeChild& child, const Eigen::Vector3f& node_position)
    {
        Eigen::Vector3f tangent = Geometry::projected_on_plane(child.node.direction, parent.direction).normalized();
        return node_position + parent.direction * parent.length * child.position_in_parent + tangent * parent.radius;
    }

    bool has_side_branches(const TreeNode& node)
    {
    ...
    void mesh_node_rec(const TreeNode& node, const Eigen::Vector3f& node_position, const CircleDesignator& base, TreeMesh& mesh, const float uv_y)
    {
        if (node.children.size() < 2)
        {
            ...
            Eigen::Vector3f child_pos = NodeUtilities::get_position_in_node(node_position, node, 1);
        ...
        else
        {
            ...
            for (int i = 0; i < node.children.size(); i++)
            {
                ...
                {
                    auto& child = *node.children[i];
                    Eigen::Vector3f child_pos = get_side_child_position(node, child, node_position);
...
namespace Mtree
{
	...
	TreeMesh ManifoldMesher::mesh_tree(Tree& tree)
    {
        ...
        auto& direction_attr = mesh.add_attribute<Eigen::Vector3f>(AttributeNames::direction);
