...
namespace Mtree
{
	std::vector<std::vector<BasicMesher::SplinePoint>> BasicMesher::get_splines(std::vector<Stem>& stems)
	{
		...
		for (Stem& stem : stems)
		{
			...
			Eigen::Vector3f  stem_position = stem.position;
	...
	void BasicMesher::get_splines_rec(std::vector<std::vector<SplinePoint>>& splines, TreeNode* current_node, Eigen::Vector3f current_position)
	{
		...
		for (size_t i = 0; i < current_node->children.size(); i++)
		{
			...
			Eigen::Vector3f child_position = current_position + current_node->direction * current_node->length * child.position_in_parent;
