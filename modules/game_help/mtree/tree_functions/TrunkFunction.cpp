...
namespace Mtree
{
	void TrunkFunction::execute(std::vector<Stem>& stems, int id, int parent_id)
	{
		...
		Eigen::Vector3f stem_direction = Eigen::Vector3f{ 0,0,1 };
		...
		for (size_t i = 0; i < node_count; i++)
		{
			...
			Eigen::Vector3f direction = current_node->direction + Geometry::random_vec() * (randomness / (resolution+.001f));
			direction += Eigen::Vector3f(0,0,up_attraction / (resolution + .001f));
			direction.normalize();
