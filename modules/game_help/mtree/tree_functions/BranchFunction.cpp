...
#include "mtree/utilities/NodeUtilities.hpp"
#include "mtree/utilities/GeometryUtilities.hpp"
...
namespace
{
	void update_positions_rec(TreeNode& node, const Eigen::Vector3f& position)
	{
		...
		{
			Eigen::Vector3f child_position = position + node.direction * node.length * child->position_in_parent;
			update_positions_rec(child->node, child_position);
		}
	}

	bool avoid_floor(const Eigen::Vector3f& node_position, Eigen::Vector3f& node_direction, float parent_length) // return true if branch should be terminated
	{
	...
	Eigen::Vector3f get_main_child_direction(TreeNode& parent, const Eigen::Vector3f& parent_position, const float up_attraction, const float flatness, const float randomness, const float resolution, bool& should_terminate)
	{
		Eigen::Vector3f random_dir = Geometry::random_vec(flatness).normalized() + Eigen::Vector3f{ 0,0,1 } * up_attraction;
		Eigen::Vector3f child_direction = parent.direction + random_dir * randomness / resolution;
	...
	Eigen::Vector3f get_split_direction(const TreeNode& parent, const Eigen::Vector3f& parent_position, const float up_attraction, const float flatness, const float resolution, const float angle)
	{
		Eigen::Vector3f child_direction = Geometry::random_vec();
		child_direction = child_direction.cross(parent.direction) + Eigen::Vector3f{ 0,0,1 } *up_attraction * flatness;
		Eigen::Vector3f flat_normal = Eigen::Vector3f{ 0,0,1 }.cross(parent.direction).cross(parent.direction).normalized();
...
namespace Mtree
{
	...
	void BranchFunction::apply_gravity_rec(TreeNode& node, Eigen::AngleAxisf curent_rotation)
	{
		...
		if (!info.inactive || true)
		{
			...
			Eigen::Vector3f tangent = node.direction.cross(Eigen::Vector3f{ 0,0,-1 }).normalized();
	...
	void BranchFunction::grow_node_once(TreeNode& node, const int id, std::queue<std::reference_wrapper<TreeNode>>& results) 
	{
		...
		Eigen::Vector3f child_direction = get_main_child_direction(node, info.position, up_attraction, flatness, randomness.execute(factor_in_branch), resolution, should_terminate);
		...
		Eigen::Vector3f child_position = info.position + child_direction * child_length;
		...
		if (split)
		{
			Eigen::Vector3f split_child_direction = get_split_direction(node, info.position, up_attraction, flatness, resolution, split_angle);
			...
			Eigen::Vector3f split_child_position = info.position + split_child_direction * child_length;
	...
	std::vector<std::reference_wrapper<TreeNode>> BranchFunction::get_origins(std::vector<Stem>& stems, const int id, const int parent_id)
	{
		...
		for (auto& branch : selection) // parent branches
		{
			...
			Eigen::Vector3f tangent = Geometry::get_orthogonal_vector(branch[0].node->direction);

			for	(size_t node_index = 0; node_index<branch.size(); node_index++)
			{
				auto& node = *branch[node_index].node;
				Eigen::Vector3f node_position = branch[node_index].node_position;
				...
				else
				{
					...
					for (int i = 0; i < origins_to_create; i++)
					{
						...
						tangent.normalize();
						Eigen::Vector3f child_direction = Geometry::lerp(node.direction, tangent, start_angle.execute(factor) / 90);
				 		child_direction.normalize();
						...
						node.children.push_back(std::make_shared<NodeChild>(std::move(child)));
						auto& child_node = node.children.back()->node;
						Eigen::Vector3f child_position = node_position + node.direction * node.length * position_in_parent;
