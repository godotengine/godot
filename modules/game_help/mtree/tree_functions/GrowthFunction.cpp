...
namespace Mtree
{
	...
	void GrowthFunction::simulate_growth_rec(TreeNode& node, int id)
	{
		...
		if (primary_growth) 
		{
			Eigen::Vector3f child_direction = node.direction + Eigen::Vector3f{0,0,1} * gravitropism + Geometry::random_vec() * randomness;
		...
		if (split)
		{
			...
			Eigen::Vector3f tangent{ std::cos(info.philotaxis_angle), std::sin(info.philotaxis_angle), 0 };
			tangent = Geometry::get_look_at_rot(node.direction) * tangent;
			Eigen::Vector3f child_direction = Geometry::lerp(node.direction, tangent, split_angle / 90);
			child_direction.normalize();
			float child_radius = node.radius;
			float child_length = branch_length * (info.vigor + .1f);
			NodeChild child = NodeChild{ TreeNode{child_direction, node.tangent, branch_length, child_radius, id}, 1 };
	...
	void GrowthFunction::get_weight_rec(TreeNode& node)
	{
		...
		Eigen::Vector3f center_of_mass = (info.absolute_position + node.direction * node.length / 2) * segment_weight;
	...
	void GrowthFunction::apply_gravity_rec(TreeNode& node, Eigen::Matrix3f curent_rotation)
	{
		BioNodeInfo& info = static_cast<BioNodeInfo&>(*node.growthInfo);
		Eigen::Vector3f offset = (info.center_of_mass - info.absolute_position);
		offset[2] = 0;
		...
		Eigen::Vector3f tangent = node.direction.cross(Eigen::Vector3f{ 0,0,-1 });
		Eigen::Matrix3f rot;
	...
	void GrowthFunction::update_absolute_position_rec(TreeNode& node, const Eigen::Vector3f& node_position)
	{
		static_cast<BioNodeInfo*>(node.growthInfo.get())->absolute_position = node_position;
		for (auto& child : node.children)
		{
			Eigen::Vector3f child_position = node_position + node.direction * child->position_in_parent * node.length;
