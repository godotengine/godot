...
namespace Mtree
{
	namespace NodeUtilities
	{
		...
		void select_from_tree_rec(BranchSelection& selection, TreeNode& node, const Eigen::Vector3f& node_position, int id)
		{
			...
			for (auto& child : node.children)
			{
				...
				first_child = false;
				Eigen::Vector3f child_position = node_position + child->node.direction * child->position_in_parent * child->node.length;
