...
namespace HumanAnim
{
    ...
    void HumanAnimmation::build_virtual_pose_global(HumanBoneConfig& p_config,Transform3D& parent_trans, BonePose& pose, HashMap<String, String>& p_human_bone_label) {
    ...
    void HumanAnimmation::build_skeleton_pose(Skeleton3D* p_skeleton, HumanBoneConfig& p_config, HumanSkeleton& p_skeleton_config, bool position_by_hip ) {
        ...
        p_skeleton->force_update_all_bone_transforms(false);
        {
            ...
            for (auto& it : p_config.root_bone) {
				...
				p_skeleton_config.root_lookat[it] = pose.get_look_at_and_roll(Transform3D(),trans.basis,local_trans);
    ...
    void HumanAnimmation::build_skeleton_lookat(Skeleton3D* p_skeleton,HumanBoneConfig& p_config, BonePose& bone_pose,Transform3D& parent_pose,HumanSkeleton& p_skeleton_config) {
        ...
        for(auto& it : bone_pose.child_bones) {
			...
			HumanAnimationBoneResult& result = p_skeleton_config.bone_result[it];
			result.bone_global_lookat = bone_pose.get_look_at_and_roll(parent_pose, trans, result.real_global_pose);
    ...
    void HumanAnimmation::build_virtual_pose(HumanBoneConfig& p_config,Skeleton3D* p_skeleton,Transform3D parent_trans,  int bone_index,HashMap<String, String>& p_human_bone_label) {
        ...
        //for(int i=0; i < child_bones.size(); i++)
        {
            ...
            Vector3 bone_forward = trans.origin - parent_trans.origin;
    ...
    void HumanAnimmation::retarget(HumanBoneConfig& p_config,BonePose& pose,Transform3D& parent_trans,HumanSkeleton& p_skeleton_config) {
