# Architecture Documentation

Use this folder for architecture-level documentation and deep technical references.

## Canonical Entry Points

- High-level architecture overview: [overview.md](overview.md)
- Detailed render pipeline architecture: [render-pipeline.md](render-pipeline.md)
- Detailed lighting and shadows architecture: [lighting-system.md](lighting-system.md)

## Module Deep References

- Module architecture map: [../../modules/gaussian_splatting/ARCHITECTURE.md](../../modules/gaussian_splatting/ARCHITECTURE.md)
- Memory subsystem and residency invariants: [../../modules/gaussian_splatting/MEMORY_SUBSYSTEM.md](../../modules/gaussian_splatting/MEMORY_SUBSYSTEM.md)
