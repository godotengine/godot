...
| ISSUE-011 | Tile Renderer | Zero-element and sort key config validation before GPU dispatch | High | agent-tile-renderer | Ready for Merge | ISSUE-010 | 78eae20b1f6 (`RenderFrameExecutor` now skips global-sort/raster dispatch for zero-work frames and clears outputs without resolve dispatch) |
| ISSUE-012 | Tile Renderer | Replace global mutex bottlenecks with RWLock read concurrency | High | agent-tile-renderer | Blocked | ISSUE-001 | Out-of-scope for tile renderer write packet (`core/gaussian_data.*`); RWLock coverage verified in tree, awaiting `agent-core-data` merge path |
| ISSUE-013 | Tile Renderer | Optional AVX2 path with safe fallback for portability | Medium | agent-tile-renderer | Ready for Merge | None | Verified `modules/gaussian_splatting/SCsub`: `gs_avx2=yes/no/auto` and `GS_AVX2_ENABLED` guarded defines/flags |
| ISSUE-014 | Tile Renderer | Wire memory leak detection into runtime test coverage | Medium | agent-tile-renderer | Ready for Merge | None | 78eae20b1f6 adds tile renderer lifecycle leak regression; existing `test_memory_leak_detection.h` coverage also present |
| ISSUE-015 | Tile Renderer | Compile-time rasterizer statistics gating via build define | Medium | agent-tile-renderer | Ready for Merge | ISSUE-014 | 78eae20b1f6 gates `GS_COLLECT_RASTER_STATS` define emission to debug/dev/tests build defines |
