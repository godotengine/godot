...
def is_class_not_included(class_name: str):
    return class_name in [
        # Only in Android builds.
        "JavaClass",
        "JavaClassWrapper",
        "JavaObject",
        # Only in web builds.
...
def generate_methods_files(reftag: str):
    """
    Parse methods specified in given Godot version to a form readable
    by compatibility checker GDExtension.
    """
    gdextension_reference_json = download_gdextension_api(reftag)

    with open(CLASS_METHODS_FILE, "w") as f:
        f.writelines(
            [
                ...
                for (klass, func) in itertools.chain(
                    (
                        ...
                        if not is_class_not_included(klass["name"])
