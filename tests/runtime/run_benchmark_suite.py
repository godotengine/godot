...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Scale all profile durations (must be > 0).",
    )
    parser.add_argument(
        "--lane",
        action="append",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Generate per-lane dummy .ply assets under project and use them for lane scenes.",
    )
    parser.add_argument(
        "--dummy-asset-dir",
        default="benchmark_assets_generated",
        help="Project-relative output directory for generated dummy .ply assets.",
    )
    parser.add_argument(
        "--no-headless-summary",
        action="store_true",
        help="Do not pass --benchmark-headless-summary to lane scenes.",
    )
    parser.add_argument(
        "--benchmark-instancing-mode",
        choices=["auto", "serial", "single_pass"],
        default="auto",
        help="Override lane instancing mode (auto/serial/single_pass).",
    )
    parser.add_argument(
        "--fail-fast",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Override LANE_TIMEOUT_SCALE (default: 3.0). Higher values give more time for heavy GPU workloads.",
    )
    parser.add_argument(
        "--timeout-grace",
        type=int,
        default=None,
        help="Override LANE_TIMEOUT_GRACE_SECONDS (default: 45). Extra seconds added for startup/import.",
    )
    parser.add_argument(
