...
func _bake_from_scene(args: Dictionary) -> GaussianSplatWorld:
    ...
    var container := _find_container(root, container_path)
    if container == null:
        return null
...
func _find_container(root: Node, container_path: String) -> GaussianSplatContainer:
    if container_path != "":
        var node := root.get_node_or_null(container_path)
        if node == null:
