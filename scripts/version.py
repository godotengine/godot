...
def __make_version_header(json_version_file: str, out_version_file: str, out_hash_file: str, built_modules: [str]):
    ...
    version_modules = [x for x in built_modules if x in modules_to_show_in_version]
    ...
    f.write('#define VERSION_MODULE_CONFIG "' + ".".join(version_array) + '"\n')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate the version header")
    ...
    __make_version_header(args.json_version_file, args.out_version_file, args.out_hash_file, args.built_modules)
